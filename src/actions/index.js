...
const logs = {
  ...
  flowStep: {
    startDebug: (flowId, exportId, minutes) => action(actionTypes.LOGS.FLOWSTEP.DEBUG.START, { flowId, exportId, minutes }),
    stopDebug: (flowId, exportId) => action(actionTypes.LOGS.FLOWSTEP.DEBUG.STOP, { flowId, exportId }),
    requestLogDetails: (flowId, exportId, logKey) => action(actionTypes.LOGS.FLOWSTEP.LOG.REQUEST, { flowId, exportId, logKey }),
    receivedLogDetails: (exportId, logKey, logDetails) => action(actionTypes.LOGS.FLOWSTEP.LOG.RECEIVED, { exportId, logKey, logDetails }),
    removeLog: (flowId, exportId, logsToRemove) => action(actionTypes.LOGS.FLOWSTEP.LOG.REMOVE, { flowId, exportId, logsToRemove }),
    logDeleted: (exportId, deletedLogKey) => action(actionTypes.LOGS.FLOWSTEP.LOG.DELETED, { exportId, deletedLogKey }),
    request: ({ flowId, exportId, loadMore }) => action(actionTypes.LOGS.FLOWSTEP.REQUEST, { flowId, exportId, loadMore }),
    received: ({ exportId, logs, nextPageURL, loadMore }) => action(actionTypes.LOGS.FLOWSTEP.RECEIVED, { exportId, logs, nextPageURL, loadMore }),
    failed: (exportId, error) => action(actionTypes.LOGS.FLOWSTEP.FAILED, { exportId, error }),
    clear: exportId => action(actionTypes.LOGS.FLOWSTEP.CLEAR, { exportId }),
    startLogsPoll: (flowId, exportId) => action(actionTypes.LOGS.FLOWSTEP.START_POLL, { flowId, exportId }),
    stopLogsPoll: (exportId, hasNewLogs) => action(actionTypes.LOGS.FLOWSTEP.STOP_POLL, { exportId, hasNewLogs }),
    setActiveLog: (exportId, activeLogKey) => action(actionTypes.LOGS.FLOWSTEP.ACTIVE_LOG, { exportId, activeLogKey }),
    setFetchStatus: (exportId, status) => action(actionTypes.LOGS.FLOWSTEP.FETCH_STATUS, { exportId, status }),
    pauseFetch: (flowId, exportId) => action(actionTypes.LOGS.FLOWSTEP.PAUSE_FETCH, { flowId, exportId }),
  },
};
