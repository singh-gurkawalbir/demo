...
jest.mock('../../components/RunFlowButton', () => ({
  __esModule: true,
  ...jest.requireActual('../../components/RunFlowButton'),
  default: () => <div>Test Run Button</div>,
}
));
jest.mock('../../components/StartDebug', () => ({
  __esModule: true,
  ...jest.requireActual('../../components/StartDebug'),
  default: () => <div>Test Start Debug Button</div>,
}
));
...
describe('Script logs', () => {
  ...
  test('Should able to test the last 15 minutes date range select button with no execution logs', async () => {
    ...
    expect(mockDispatchFn).toHaveBeenCalledWith(actions.logs.scripts.patchFilter({
      ...
      value: {
        ...
        startDate: new Date('2022-08-12T23:45:00.000Z'),
      },
    }));
    expect(applyButtonNode).not.toBeInTheDocument();
  }, 30000);
  test('Should able to test the last 24 hours date range select button', async () => {
    ...
    await waitFor(() => expect(mockDispatchFn).toHaveBeenCalledWith(actions.logs.scripts.patchFilter({
      ...
      value: {
        endDate: null,
        preset: 'last24hours',
        startDate: new Date('2022-08-12T00:00:00.000Z'),
      },
    })));
    expect(applyButtonNode).not.toBeInTheDocument();
  }, 30000);
  test('Should able to test the last 30 minutes date range select button', async () => {
    ...
    expect(mockDispatchFn).toHaveBeenCalledWith(actions.logs.scripts.patchFilter({
      ...
      value: {
        ...
        startDate: new Date('2022-08-12T23:30:00.000Z'),
      },
    }));
    expect(applyButtonNode).not.toBeInTheDocument();
  }, 30000);
  test('Should able to test the last hour date range select button', async () => {
    ...
    expect(mockDispatchFn).toHaveBeenCalledWith(actions.logs.scripts.patchFilter({
      ...
      value: {
        endDate: null,
        preset: 'last1hour',
        startDate: new Date('2022-08-12T23:00:00.000Z'),
      },
    }));
    expect(applyButtonNode).not.toBeInTheDocument();
  }, 30000);
  test('Should able to test the yesterday date range select button', async () => {
    ...
    expect(mockDispatchFn).toHaveBeenCalledWith(actions.logs.scripts.patchFilter({
      ...
      field: 'dateRange',
      value: {
        endDate: new Date('2022-08-12T23:59:59.999Z'),
        preset: 'yesterday',
        startDate: new Date('2022-08-12T00:00:00.000Z'),
      },
    }));
  ...
  test('Should able to test the last 4 hours date range select button', async () => {
    ...
    expect(mockDispatchFn).toHaveBeenCalledWith(actions.logs.scripts.patchFilter({
      ...
      value: {
        endDate: null,
        preset: 'last4hours',
        startDate: new Date('2022-08-12T20:00:00.000Z'),
      },
    }));
    expect(applyButtonNode).not.toBeInTheDocument();
  }, 30000);
  ...
  test('Should able to test the run now button', async () => {
    ...
    store(sessionLogs);
    await initScriptLogs(props);
    const runNowNode = screen.getByText('Test Run Button');

    expect(runNowNode).toBeInTheDocument();
  });
  test('Should able to test the start debug button', async () => {
    ...
    store(sessionLogs);
    await initScriptLogs(props);
    const testStartDebugButton = screen.getByText('Test Start Debug Button');

    expect(testStartDebugButton).toBeInTheDocument();
  });
