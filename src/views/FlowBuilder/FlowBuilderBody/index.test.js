...
function initFlowBuilderBody(props = {}) {
  ...
  initialStore.getState().session.flowbuilder = {'62c6f122a2f4a703c3dee3d0': {dragStepIdInProgress: true, status: props.status}};
...
jest.mock('react-flow-renderer', () => ({
  ...
  default: props => (
    <div>
      ...
      <><button type="button" onClick={props.onNodeDragStart}>DragStart</button>
        <button type="button" onClick={props.onNodeDragStop}>DragStop</button>
        <button type="button" onClick={props.onNodeDrag}>NodeDrag</button>
...
describe('FlowBuilderBody UI tests', () => {
  ...
  test('should pass the initial render with all the available operations', () => {
    ...
    const flowToggleButton = document.querySelector('[ data-test="switchFlowOnOff"]');

    expect(flowToggleButton).toBeInTheDocument();
    const scheduleFlowButton = document.querySelector('[ data-test="scheduleFlow"]');

    expect(scheduleFlowButton).toBeInTheDocument();
    const settingsButton = document.querySelector('[ data-test="flowSettings"]');
  ...
  test('should display the zoomin and zoomout options for miniMap', () => {
    const props = {flowId: '62c6f122a2f4a703c3dee3d0', fullScreen: true, status: 'saving' };

    initFlowBuilderBody(props);
    const ZoomInButton = document.querySelector('[ class="react-flow__controls-button react-flow__controls-zoomin"]');

    expect(ZoomInButton).toBeInTheDocument();
    const ZoomOutButton = document.querySelector('[ class="react-flow__controls-button react-flow__controls-zoomout"]');

    expect(ZoomOutButton).toBeInTheDocument();
    const FitViewButton = document.querySelector('[ class="react-flow__controls-button react-flow__controls-fitview"]');
  ...
  test('should make the respective dispatch call when clicked on SourceTitle', () => {
    ...
    expect(mockDispatchFn).toBeCalledWith(actions.flow.addNewPGStep('62c6f122a2f4a703c3dee3d0'));
  });
});
