...
const getIcon = index => {
  const Symbols = [RequiredIcon, OptionalIcon, ConditionalIcon, PreferredIcon];
...
const Chart = ({ id, flowId, range, selectedResources }) => {
  ...
  const CustomizedDot = props => {
    ...
    if (value) {
      return (
        <Icon
          {...props} x={cx - 5} y={cy - 5} width={size}
          height={size} />
      );
      // return (
      //   <svg
      //     {...props}
      //     x={cx - 5} y={cy - 5} width={size} height={size}
      //     viewBox="0 0 9 10">
