...
export default (state = {}, action) => {
  ...
  return produce(state, draft => {
    ...
    switch (type) {
      case actionTypes.INTEGRATION_APPS.INSTALLER.V2.STEP.DONE:
        draft[id] = {openOauthConnection: draft[id]?.openOauthConnection, connectionId: draft[id]?.connectionId};

        break;
      case actionTypes.INTEGRATION_APPS.INSTALLER.V2.RECEIVED_OAUTH_CONNECTION_STATUS:
        draft[id] = {};
        draft[id].openOauthConnection = openOauthConnection;
        draft[id].connectionId = connectionId;
        break;

      case actionTypes.INTEGRATION_APPS.INSTALLER.V2.STEP.UPDATE:
        if (!draft[id]) {
          draft[id] = {};
        }
...
selectors.v2integrationAppsInstaller = (state, id) => {
  if (!state || !state[id]) {
    return emptyObj;
  }
...
selectors.v2canOpenOauthConnection = (state, id) => {
  if (!state || !state[id]) {
