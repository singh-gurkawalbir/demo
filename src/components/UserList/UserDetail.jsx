...
class UserDetail extends Component {
  ...
  getMessageForAction(action, commStatus, user) {
    ...
    if (action === 'disable') {
      ...
      if (commStatus.status === COMM_STATES.SUCCESS) {
        message = `An Account Ownership invitation has been sent to ${user.sharedWithUser.name} (${user.sharedWithUser.email}).<br/>Once accepted, your account will be converted to a regular user account with Manager access.`;
      } else if (commStatus.status === COMM_STATES.ERROR) {
