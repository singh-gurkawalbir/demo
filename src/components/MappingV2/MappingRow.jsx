...
export default function MappingRow(props) {
  ...
  const generateFields = useSelector(state =>
    selectors.mappingV2Generates(state, resourceId)
  );
  const nsRecordType = useSelector(state =>
    selectors.mappingV2NSRecordType(state, resourceId, subRecordMappingId)
  );

  const extractFields = useSelector(state =>
    selectors.mappingV2Extracts(state, resourceId, flowId)
  );
  const {lookups, lastModifiedRowKey} = useSelector(state => selectors.mappingV2(state));
  const updateLookupHandler = (lookupOps = []) => {
    ...
    dispatch(actions.mappingV2.updateLookup(lookupsTmp));
  };
  ...
  const handleBlur = useCallback(
    field => (id, value) => {
      ...
      if (field === 'extract' && value.indexOf('_child_') > -1) {
        dispatch(actions.mappingV2.checkForSFSublistExtractPatch(key, value));
      ...
      if ((!key && value) || (key && mapping[field] !== value)) {
        if (key && value === '') {
          if (
            ...
            dispatch(actions.mappingV2.delete(key));

            return;
          }
        }
        dispatch(actions.mappingV2.patchField(field, key, value));
      ...
      if (lastModifiedRowKey !== key) {
        ...
        dispatch(actions.mappingV2.updateLastFieldTouched(_lastModifiedRowKey));
      }
    },
  ...
  const handleSettingsSave = useCallback(
    (id, value) => {
      dispatch(actions.mappingV2.patchSettings(key, value));
    },
    [dispatch, key]
  );
