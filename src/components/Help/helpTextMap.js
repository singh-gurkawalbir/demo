...
export default {
  formView:
    'The application specific form is customized to help configure your resource for this particular application. However, if you would like more flexibility, choose the universal connector form (e.g. REST or HTTP) instead, which is a generic form.',
  'afe.handlebar.data': 'This is sample help for handlebar afe, data panel',
  'afe.handlebar.rule.relaviveUri':
    'This is sample help for handlebar afe, rule panel, relativeUri fieldId',
  previewPanelRecords:
    'Enter or select the maximum number of records – up to 100 – to preview. Then, click <b>Preview</b> to see the records that match your query in the order that they’re received.',
  settingsForm:
  ...
  'license._trialLicenseId':
    'Create or select a license record that would serve as the default license configuration for any trial licenses.',
  'license.trialEnabled':
    'You must create the integration app listing before you can enable the trials.',
  // fieldDefinitions
  ...
  'import.rdbms.query':
    'Enter the exact SQL query to be sent to your database. The query can be static or dynamic, which means that <a href="https://docs.celigo.com/hc/en-us/articles/360039326071">handlebars</a> templates are used to source data and settings from your flow.',
  // definitions
  'amazonmws.connection.http.sellingPartnerId':
    'The identifier of the selling partner who is authorizing your application.',
  'amazonmws.connection.http.type':
    '<b>Selling Partner API (SP-API)</b>: The Selling Partner API is a REST-based API and is an evolution of the legacy Amazon Marketplace Web Service (MWS) APIs. It’s recommended you integrate using SP-APIs.<br><b>Hybrid Selling Partner API (SP-API and MWS)</b>: Hybrid Selling Partner API application can make calls both to the Selling Partner API and to Amazon Marketplace Web Service (Amazon MWS). Use Hybrid Selling Partner API when your integration requires functionality from both services.<br><b>Marketplace Web Service API (MWS)</b>: Amazon Marketplace Web Service (Amazon MWS) is the legacy web service API.',
  ...
  'omnisend.connection.http.auth.token.token':
  ...
  'pagerduty.connection.http.auth.token.token':
  ...
  'vroozi.connection.http.auth.token.token':
    'Please enter your Access Token here. To get an API key for your Vroozi account,login to your Vroozi account and Under API Integration section, click on Credentials.Create your application by clicking on "Add New Application" After entering your application name, you will be provided with an access token which is shown only once.<br>Multiple layers of protection, including AES 256 encryption, are in place to keep your Access token safe. When editing this connection, you must re-enter this value each time; it is stored only when the connection is saved and never displayed as text.',
  'saplitmos.connection.http.auth.token.token':
    'Please enter API Key of your SAP Litmos Account. <br> <b>Steps to generate API Key:</b> <br> 1. Login to SAP Litmos Account <br>2. Click "My Profile & Settings" from the drop-down menu at the top right corner of the screen <br>3. View the bottom of your profile.<br>Multiple layers of protection, including AES 256 encryption, are in place to keep your API key safe. When editing this connection, you must re-enter this value each time; it is stored only when the connection is saved and never displayed as text.',
  'oandav20fxtrade.connection.accountType':
  ...
  'connection.http.oneloginRegion':
    'OneLogin is hosted in two independent operating regions: the United States (US) and the European Union (EU). Enter your location as the region.',
  ...
  'okta.connection.http.auth.token.token':
    'Please enter your Token here. To generate an API key for a given user, users should log in and click their name in the upper right hand corner of any page to get to the user context menu. There will be an "API Keys" option in that menu to go to the page.<br>Multiple layers of protection, including AES 256 encryption, are in place to keep your token safe. When editing this connection, you must re-enter this value each time; it is stored only when the connection is saved and never displayed as text.',
  'pipedrive.connection.http.auth.type': 'Please select Authentication Type.',
  'pipedrive.connection.http.subdomain':
    'Please enter the unique portion of the pipedrive.com address that you visit to log in to your portal. For example, if your portal is found at https://personnel.pipedrive.com, then enter personnel for the subdomain.',
  'pipedrive.connection.http.auth.token.token':
  ...
  'zohocrm.connection.http.zohoSubdomain':
  ...
  'recurly.connection.http.unencrypted.version':
    'The API versions determines which API your connection will use.<br><b>Note:</b>The v2 APIs are going to be deprecated; use the v3 instead.',
  ...
  'amazonmws.connection.http.unencrypted.sellerId':
  ...
  'confluencecloud.connection.http.unencrypted.instanceURL':
    'Enter your Confluence Cloud instance URL. If your account is located at https://demo.atlassian.net/wiki/home, enter <b>demo.atlassian.net</b>.',
  'confluencecloud.connection.http.auth.basic.username':
    'The username of your Confluence Cloud account.',
  'confluencecloud.connection.http.auth.basic.password':
  ...
  '3dcart.connection.http.auth.token.token':
    "The 3dcart merchant's token.<br>Multiple layers of protection, including AES 256 encryption, are in place to keep your token safe. When editing this connection, you must re-enter this value each time; it is stored only when the connection is saved and never displayed as text.",
  ...
  'anaplan.connection.http.auth.token.token':
  ...
  'slack.connection.http.auth.token.token':
    'Enter the token of your Slack account. <b>Steps to get the token:</b><br>1. Sign into your Slack account.<br>2. Click <b>Your Apps</b> in the top right corner and select an application.<br>3. Navigate to the <b>OAuth & Permissions tab</b>. If there are no tokens, you will get an option to <b>Install to Workspace</b>.<br>The tokens will be automatically generated once the app is connected to the workspace.<br><br> Multiple layers of protection are in place, including AES 256 encryption, to keep your connection’s token safe. When editing this form later, you must enter this value again; it is stored only when the connection is saved and never displayed as text.',
  'slack.import.resource':
    'Choose the API resource to send the records to Slack, such as <b>chat</b>, <b>groups</b>, or <b>team</b>. The methods below will change according to the resource you apply.',
  'slack.import.operation':
    'Choose the Slack API method for the selected resource. For example, if you’re posting a field to a specific channel, select <b>chat</b> for the resource and <b>chat.postMessage</b> for the method.',
  ...
  'ebay.connection.http.unencrypted.apiType':
    'Select your API type here. <br>1. Select <b>Finance API</b> if the requirement is for Finance API. <br>2. Select <b>Other API</b> for all other APIs like, Account API, Compliance API, Analytics API, Inventory API, Fulfillment API, Metadata API.',
  ...
  'servicenow.connection.http._iClientId':
  ...
  'shopify.connection.http.auth.type':
  ...
  'shopify.connection.http.storeURL':
    'Enter the unique portion of your Shopify store’s URL. For example, if your Shopify store URL is https://demo-store.myshopify.com, then provide <i>demo-store</i> for the store URL.',
  ...
  'shopify.connection.http.auth.basic.username':
  ...
  'shopify.connection.http.auth.basic.password':
    'Login to your Shopify store and navigate to "Apps" section. Click on the respective private app and the password can be found next to the "Authentication" section.<br>Multiple layers of protection, including AES 256 encryption, are in place to keep your password safe. When editing this connection, you must re-enter this value each time; it is stored only when the connection is saved and never displayed as text.',
  'concurinvoice.connection.http.unencrypted.username':
    'Please enter the value of <b>id</b> which appears in the redirected popup page URL after signin to integrator.io.',
  'concurinvoice.connection.http.encrypted.password':
    'Please enter the value of <b>requestToken</b> which appears in the redirected popup page URL after signin to integrator.io.<br>Multiple layers of protection, including AES 256 encryption, are in place to keep your password safe. When editing this connection, you must re-enter this value each time; it is stored only when the connection is saved and never displayed as text.',
  'concurinvoice.connection.http.auth.token.token':
  ...
  'concurexpense.connection.http.auth.token.token':
  ...
  'fulfilment.connection.http.unencrypted.clientId':
    'Please enter Client ID of your Fulfillment.com account. <br> Note: To obtain an OAuth Client ID contact your account executive.',
  'fulfilment.connection.http.encrypted.clientSecret':
    'Please enter Client Secret of your Fulfillment.com account. <br>Note: To obtain an OAuth Client Secret contact your account executive.<br>Multiple layers of protection, including AES 256 encryption, are in place to keep your secret safe. When editing this connection, you must re-enter this value each time; it is stored only when the connection is saved and never displayed as text.',
  'fulfilment.connection.http.unencrypted.username':
    'Enter the username of your Fulfillment.com account.',
  'fulfilment.connection.http.encrypted.password':
    'Enter the password of your Fulfillment.com account.<br>Multiple layers of protection, including AES 256 encryption, are in place to keep your password safe. When editing this connection, you must re-enter this value each time; it is stored only when the connection is saved and never displayed as text.',
  'fulfilment.connection.http.auth.token.token':
  ...
  'woocommerce.connection.http.baseURI':
    'Enter the base URI of your WooCommerce account. For example, if your endpoint is at https://shopName.com/wp-json/wc/v1/orders, then enter https://shopName.com.',
  ...
  'tableau.connection.http.unencrypted.version':
    'Please enter the version of the API to use, such as 3.10. For more information, see <a href="https://help.tableau.com/current/api/rest_api/en-us/REST/rest_api_concepts_versions.htm">REST API and Resource Versions</a>.',
  ...
  'taxjar.connection.http.auth.token.token':
    'Please enter your token here. This can be obtained by navigating to Tokens page from the options menu on the top right corner in the application.<br>Multiple layers of protection, including AES 256 encryption, are in place to keep your token safe. When editing this connection, you must re-enter this value each time; it is stored only when the connection is saved and never displayed as text.',
  'pitneybowes.connection.http.sandbox': 'Please select your environment here.',
  ...
  'mixpanel.connection.http.unencrypted.projectId':
    "Enter your Mixpanel Project ID.<br><b>Steps to get the Project ID: </b><br>1. Sign in to your Mixpanel Account.<br>2. Navigate to <b>Settings</b> → <b>Project settings</b>.<br>3. Select your project (if none exists yet, create a new project).<br>4. Copy your <b>Project ID</b> from the 'Overview' tab.",
  'mixpanel.connection.http.encrypted.projectToken':
    "Enter your Mixpanel Project token.<br><b>Steps to get the Project Token: </b><br>1. Sign in to your Mixpanel Account.<br>2. Navigate to <b>Settings</b> → <b>Project settings</b>.<br>3. Select your project (if none exists yet, create a new project).<br>4. Copy your <b>Project Token</b> from the 'Overview' tab.<br><br>Multiple layers of protection, including AES 256 encryption, are in place to keep your token safe. When editing this connection, you must re-enter this value each time; it is stored only when the connection is saved and never displayed as text.",
  'mixpanel.connection.http.auth.basic.username':
    "Enter your Mixpanel username.<br><b>Steps to get the username: </b><br>1. Sign in to your Mixpanel Account.<br>2. Navigate to <b>Settings</b>  →  <b>Organizational Settings</b> and select an organization.<br>3.Select your service account(if none exists yet, create a new service account).<br>4. Copy your <b>Username</b> from the 'Service account' tab.",
  'mixpanel.connection.http.auth.basic.password':
  ...
  'saphana.connection.http.unencrypted.host':
    'Enter the host for your SAP S/4HANA Cloud instance. If your instance is located at <b>https://my1234.s4hana.ondemand.com</b>, then enter <b>my1234</b>.<br> This URL can also be found under the "Communication Arrangement" tab → "API-URL" field in your SAP S/4HANA Cloud instance.',
  'saphana.connection.http.auth.basic.username':
    'Enter the username from the Communication Arrangement.<br> For more information, refer to <a href=https://blogs.sap.com/2017/11/09/setting-up-communication-management-in-sap-s4hana-cloud/>Setting up Communication Management in SAP S/4HANA Cloud</a>',
  'saphana.connection.http.auth.basic.password':
    'Enter the password from the Communication Arrangement.<br><br>Multiple layers of protection, including AES 256 encryption, are in place to keep your password safe. When editing this connection, you must re-enter this value each time; it is stored only when the connection is saved and never displayed as text.',
  'zenefits.connection.http.auth.token.token':
    'Enter your Zenefits App key.<br><b>Steps to get the App key: </b><br>1.Log in to your Zenefits account.<br>2.On the dashboard, Navigate to <b>Company Profile</b> → <b>Custom Integrations</b>.<br>3. Unmask and copy the token. (If none exists yet, click “Add Token” and select required scopes to create a new token.)<br><br>Multiple layers of protection, including AES 256 encryption, are in place to keep your key safe. When editing this connection, you must re-enter this value each time; it is stored only when the connection is saved and never displayed as text.',
  'sapsuccessfactors.connection.http.unencrypted.baseurl':
    'Enter the Base URL for your SAP SuccessFactors account.<br>If your instance is located at <b>https://pmsalesdemo8.successfactors.com</b>, then your URL would be <b>apisalesdemo8.successfactors.com</b>.<br>For more information on DCs and API Servers, refer to <a href="https://help.sap.com/viewer/d599f15995d348a1b45ba5603e2aba9b/2105/en-US/af2b8d5437494b12be88fe374eba75b6.html">List of SAP SuccessFactors API Servers</a>.',
  'sapsuccessfactors.connection.http.unencrypted.clientId':
    'Enter your Client ID.<br><b>Steps to get the Client ID:</b> <br>1. Log in to your SAP SuccessFactors account. <br> 2. Navigate to your Admin Center and under tools, search for “Manage OAuth2 Client Applications”.<br>3. Register a Client Application and generate X.509 Certificate in your application or via terminal.<br>4. Click "Download" to download the certificate and the private key. (which can only be seen while registering an app)<br>5. Once registered, you can find your Client ID under the "API Key" field.',
  'sapsuccessfactors.connection.http.unencrypted.companyId':
    'Enter your Company ID, which you can find under your profile dropdown in "Show Version information", when logged in.',
  'sapsuccessfactors.connection.http.unencrypted.userId':
    'Enter Your SAP SuccessFactors User ID.',
  'sapsuccessfactors.connection.http.encrypted.assertion':
    'Once you register an application in the <b>API center</b>, you can download the private key, using which you can generate a SAML Assertion via terminal. For more information, refer to <a href=https://launchpad.support.sap.com/#/notes/3031657>How to generate SAML assertion for SAP SuccessFactors API</a><br><br>Multiple layers of protection, including AES 256 encryption, are in place to keep your assertion safe. When editing this connection, you must re-enter this value each time; it is stored only when the connection is saved and never displayed as text.',
  'sapsuccessfactors.connection.http.auth.token.token':
  ...
  'pagerdutyevents.connection.http.encrypted.routingKey':
  ...
  'quickbase.connection.http.unencrypted.hostname':
    'Enter the hostname for your Quickbase account. For example, if your account is located at https://demo.quickbase.com/, then your hostname would be <b>demo.quickbase.com</b>',
  'quickbase.connection.http.unencrypted.appId':
    'Enter the App ID. <br><b>Steps to get the App ID:</b><br>1. Sign in to your Quickbase account.<br>2. Navigate to <b>My Apps</b> and select an application(If none exists yet, create a new app).<br>3. Copy the <b>App ID</b> from your address bar.',
  'quickbase.connection.http.auth.token.token':
    'Enter the token of your Quickbase account.<br> <b>Steps to get the token:</b> <br>1. Sign in to your Quickbase Account.<br>2. Navigate to <b>My profile</b> > <b>My Preferences</b> > <b>Manage User Tokens</b>.<br>3. Copy your Token(If none exists yet or you want to change the token, Click <b>New token</b> and select appropriate permissions to create a new token).<br><br>Multiple layers of protection, including AES 256 encryption, are in place to keep your token safe. When editing this connection, you must re-enter this value each time; it is stored only when the connection is saved and never displayed as text.',
  'orangehrm.connection.http.unencrypted.username':
    'Enter the username of your OrangeHRM account.',
  'orangehrm.connection.http.encrypted.password':
    'Enter the password of your OrangeHRM account.<br>Multiple layers of protection, including AES 256 encryption, are in place to keep your password safe. When editing this connection, you must re-enter this value each time; it is stored only when the connection is saved and never displayed as text.',
  'orangehrm.connection.http.unencrypted.subdomain':
    'Enter the subdomain of your OrangeHRM account. For example, if <b>https://mycompany.orangehrm.com</b> is the login URL, then <b>"mycompany"</b> is the subdomain.',
  'pricefx.connection.http.unencrypted.baseurl':
    'Enter the Base URL for your PriceFx account. For example, if your account is located at https://test.pricefx.com/, then enter “test.pricefx.com”.<br>For more information, refer to <a href="https://pricefx.atlassian.net/wiki/spaces/KB/pages/2564915288/REST+API+Documentation#RESTAPIDocumentation-HowtoAccesstheAPI">How to Access the API</a>.',
  'pricefx.connection.http.unencrypted.partition':
    'Please enter the partition of your Pricefx account.This can be found under the Administration/User Admin section of your Pricefx account.',
  'pricefx.connection.http.unencrypted.username':
    'Please enter the username of your Pricefx account.',
  'pricefx.connection.http.encrypted.password':
    'Please enter the password of your Pricefx account.<br>Multiple layers of protection, including AES 256 encryption, are in place to keep your password safe. When editing this connection, you must re-enter this value each time; it is stored only when the connection is saved and never displayed as text.',
  ...
  'braintree.connection.http.unencrypted.environment':
    'Select the environment of your Braintree account.',
  'braintree.connection.http.auth.basic.username':
  ...
  'braintree.connection.http.auth.basic.password':
    'Enter the Private key of your Braintree account.<br> <b>Steps to get the Private Key:</b> <br>1. Sign in to your Braintree account.<br>2. Navigate to <b>My User</b> at the top right.<br>2. Click on <b>View Authorizations</b>.Copy the Private Key shown on the dashboard. (If none exists yet or you want to change the API credentials, click on <b>Generate New API Key</b> to generate a new pair.)<br>Multiple layers of protection, including AES 256 encryption, are in place to keep your key safe. When editing this connection, you must re-enter this value each time; it is stored only when the connection is saved and never displayed as text.',
  'myobadvanced.connection.http.auth.type':
    'Please select Authentication Type.',
  ...
  'g2.connection.http.auth.token.token':
  ...
  'mailgun.connection.http.auth.basic.password':
    'Please enter your API key here. This can be obtained from the Settings section and API Keys subsection.<br>Multiple layers of protection, including AES 256 encryption, are in place to keep your API key safe. When editing this connection, you must re-enter this value each time; it is stored only when the connection is saved and never displayed as text.',
  'procurify.connection.http.procurifySubdomain':
  ...
  'procurify.connection.http.encrypted.password':
    'Please enter password of your Procurify account.<br>Multiple layers of protection, including AES 256 encryption, are in place to keep your password safe. When editing this connection, you must re-enter this value each time; it is stored only when the connection is saved and never displayed as text.',
  'procurify.connection.http.generateClientIdandSecret':
  ...
  'procurify.connection.http.encrypted.clientSecret':
    'Please click "Generate client id & secret" button to get Client secret of your Procurify account.<br>Multiple layers of protection, including AES 256 encryption, are in place to keep your secret safe. When editing this connection, you must re-generate this value each time; it is stored only when the connection is saved and never displayed as text.',
  ...
  'coupa.connection.http._iClientId':
  ...
  'sugarcrm.connection.sugarcrmSubdomain':
  ...
  'sugarcrm.connection.http.unencrypted.clientID':
  ...
  'sugarcrm.connection.http.unencrypted.platform':
  ...
  'sugarcrm.connection.http.encrypted.clientSecret':
  ...
  'copper.connection.http.auth.token.token':
  ...
  'copper.connection.http.unencrypted.email':
    'Enter the email address that you use to login to Copper.',
  ...
  'microsoftpowerautomate.connection.http.unencrypted.tenantId':
    'Enter the Tenant ID of your Azure application.<br><br><b>Steps to get the Tenant ID:</b><br>1. Sign in to your <a href=https://portal.azure.com/signin/index/>Azure portal</a>.<br>2. Under <b>Azure services</b>, click on <b>App registrations</b>.<br>3. Select the application which you will be using for Microsoft Power Automate.<br>4. In the overview tab under Essentials, you will find your <b>Directory (tenant) ID</b>.',
  'microsoftpowerautomate.connection.http._iClientId':
  ...
  'googlecontacts.connection.http.unencrypted.apiType':
    'The API type determines which API your connection will use. The Contacts API is being deprecated; use the People API instead.',
  'googlecontacts.connection.http.scopePeople':
  ...
  '15five.connection.http.auth.token.token':
  ...
  'docebo.connection.http.baseURI':
    "Enter the base URI of your Docebo account. For example, if <b>https://yoursubdomain.docebosaas.com/learn/signin</b> is the login URL, then <b>'https://yoursubdomain.docebosaas.com'</b> is the base URI",
  'breezyhr.connection.http.unencrypted.email':
    'Enter the email of your Breezy HR account.',
  'breezyhr.connection.http.encrypted.password':
    'Enter the password of your Breezy HR account.<br>Multiple layers of protection, including AES 256 encryption, are in place to keep your password safe. When editing this connection, you must re-enter this value each time; it is stored only when the connection is saved and never displayed as text.',
  'breezyhr.connection.http.auth.token.token':
  ...
  'mercadolibre.connection.http._iClientId':
  ...
  'mercadolibre.connection.http.unencrypted.region':
    'Select the region of your MercadoLibre account.',
  'constantcontact.connection.http._iClientId':
    'Save your client ID and client secret in this field for an extra layer of security.<br><br><b>Steps to get the client ID and secret:<br></b>1. Open the V3 API Developer Portal and click the <b>My Applications</b> tab.<br>2. Enter your <b>username</b> and <b>password</b> and click <b>Login</b>.<br>3. Select the app you’re using to copy the client ID and secret. If none exist, click <b>New Application</b> to create a new client ID and secret.',
  ...
  'brex.connection.http.unencrypted.accountType':
  ...
  'ware2go.connection.http.platformversion':
    'Select your Ware2Go account platform version:<br>1. Select <b>Platform 1</b> to connect using the <b>Access token</b> and <b>Access secret</b> (or) select <b>Platform2</b> to connect using a <b>Username</b> and <b>Password</b>.',
  'ware2go.connection.http.unencrypted.username':
    'Please enter the username of your Ware2Go account here.',
  'ware2go.connection.http.unencrypted.password':
    'Please enter the password of your Ware2Go account here.<br>Multiple layers of protection, including AES 256 encryption, are in place to keep your password safe. When editing this connection, you must re-enter this value each time; it is stored only when the connection is saved and never displayed as text.',
  ...
  'vend.connection.http.subdomain':
    "Enter your Vend subdomain. For example, in https://soda.vendhq.com, 'soda' is the subdomain.",
  ...
  'miva.connection.http.unencrypted.signature':
    'Select the HMAC signature type.<br><b> Note:</b> Please select MIVA (no hmac present), if in the OAuth App <b>"Accept Requests Without Signature"</b> is enabled inside <b>Signature</b> section. ',
  'miva.connection.http.encrypted.privateKey':
    'Enter the Private key of your Miva account.<br><b>Steps to get Private key:</b><br> 1. Sign in to the Miva account.<br>2.On the Dashboard, select <b>Settings → User Management → API Tokens</b>.<br>3. Click on the App to edit API Token.<br>4. Under section <b>Signature</b> select the <b>Require Signature with Key</b> and click on <b>Generate</b> to get a new Private Key.<br>Multiple layers of protection, including AES 256 encryption, are in place to keep your Private key safe. When editing this connection, you must re-enter this value each time; it is stored only when the connection is saved and never displayed as text.',
  ...
  'sapbusinessone.connection.http.unencrypted.serverNameOrIP':
    'Enter the Server name/IP. Contact your SAP Admin for further details.',
  'sapbusinessone.connection.http.unencrypted.port':
    'Enter the Port number. Contact your SAP Admin for further details.',
  'sapbusinessone.connection.http.unencrypted.companyDatabase':
    'Enter the Company Database. Contact your SAP Admin for further details.',
  'sapbusinessone.connection.http.unencrypted.userName':
    'Enter the Username. Contact your SAP Admin for further details.',
  'sapbusinessone.connection.http.encrypted.password':
    'Enter the Password. Contact your SAP Admin for further details.<br>Multiple layers of protection, including AES 256 encryption, are in place to keep your password safe. When editing this connection, you must re-enter this value each time; it is stored only when the connection is saved and never displayed as text.',
  'bartender.connection.http.unencrypted.printPortalBaseURL':
    'Enter the Print portal Base URL for BarTender. For example, if your print portal is located at http://localhost/BarTender, enter <b>http://localhost</b>.',
  'bartender.connection.http.unencrypted.userName':
    'Enter the username for your BarTender print portal.',
  'bartender.connection.http.encrypted.password':
    "Enter the password for your BarTender print portal.<br>Multiple layers of protection are in place, including AES 256 encryption, to keep your connection's password safe. When editing this form later, you must enter this value again; it is stored only when the connection is saved and never displayed as text.",
  'bartender.connection.http.auth.token.token':
  ...
  'zohoexpense.connection.http._iClientId':
...
<br>Multiple layers of protection, including AES 256 encryption, are in place to keep your API key safe. When editing this connection, you must re-enter this value each time; it is stored only when the connection is saved and never displayed as text.`,
  'clover.connection.http.unencrypted.environment':
    'Please select the environment of your Clover account.',
  'clover.connection.http.unencrypted.region': 'Please select your region.',
  'clover.connection.http.unencrypted.merchantId':
    'Enter the Merchant ID for your Clover account. Use the following steps to find your Merchant ID:<br> 1. Sign in to your Clover account (or return to the <b>Businesses</b> dashboard if already signed in).<br> 2. From the <b>Setup</b> menu, select <b>Merchants</b>.<br> 3. Copy the ID shown under the merchant.',
  ...
  'orderful.connection.http.unencrypted.version':
    'Please enter the version of your Orderful account.',
  ...
  'gorgias.connection.gorgiasSubdomain':
    "Please enter your Gorgias subdomain. For example, in https://temp-portal.gorgias.com 'temp-portal' is the subdomain.",
  ...
  'paylocity.connection.http.unencrypted.environment':
    'Please select the environment of your Paylocity account.',
  'paylocity.connection.http.unencrypted.companyId':
    'Enter the Company ID.<br><br><b>Steps to get the Company ID:</b><br> 1. Sign in to the Paylocity account.<br>2.On the Dashboard, select <b>Help</b> → <b>About/Copyright</b>.<br>3. Copy the Company ID.',
  'freshservice.connection.http.unencrypted.subdomain':
    "Enter the subdomain of your Freshservice account. For example, if https://mycompany.freshservice.com is the login URL, then 'mycompany' is the subdomain.",
  'freshservice.connection.http.auth.basic.username':
  ...
  'logisense.connection.http.auth.token.token':
  ...
  'microsoftteams.connection.http.unencrypted.tenantId':
    'Please enter the Tenant ID of your Azure application.<br><b>Please follow the steps to find the Tenant ID:</b><br>1. Login to your Azure portal.<br>2. Under Azure services, click on App registrations.<br>3. Select the application which you will be using for Microsoft Teams.<br>4. In the overview tab under Essentials, you will find your Directory (tenant) ID.',
  'prestashop.connection.storeURL':
    "Please enter the URL where you log in to your back office. For example, if http://localhost/prestashop/admin12345 is your login URL, then just enter 'localhost/prestashop'.",
  'prestashop.connection.http.encrypted.apiKey':
  ...
  'adobesign.connection.http.unencrypted.subdomain':
    "Please enter the “shard” of your Adobe Sign account.For example, in https://na3.adobesign.com 'na3' is the subdomain.",
  'adobesign.connection.http.unencrypted.email':
    'Please enter the email of API caller using the Adobe Sign account.',
  ...
  'yotpo.connection.http.unencrypted.clientId':
  ...
  'yotpo.connection.http.auth.token.token':
  ...
  'yotpo.connection.http.encrypted.clientSecret':
  ...
  'googleads.connection.http.unencrypted.loginCustomerId':
    'Enter the Login customer ID of your Google Ads account. For Google Ads API calls made by a manager to a client account, you must supply the login-customer-id. This value represents the Google Ads customer ID of the manager making the API call.<br> <b>Note:</b> If you are making API calls directly to a manager account then enter the same value as <b>Customer ID</b>.',
  'googleads.connection.http.unencrypted.version':
    'Please enter your current Google Ads version.',
  ...
  'pandadoc.connection.http.auth.token.token':
  ...
  'sapbydesign.connection.http.unencrypted.tenantHostname':
    'Enter the Tenant hostname for your SAP Business ByDesign instance. If your instance is located at <b>https://my1234.sapbydesign.com</b>, then enter <b>my1234</b>.',
  'sapbydesign.connection.http.unencrypted.username':
    'Please enter the Username of your SAP ByDesign account.',
  'sapbydesign.connection.http.encrypted.password':
    'Please enter the Password of your SAP ByDesign account.<br>Multiple layers of protection, including AES 256 encryption, are in place to keep your Password safe. When editing this connection, you must re-enter this value each time; it is stored only when the connection is saved and never displayed as text.',
  'microsoftbusinesscentral.connection.http.unencrypted.apiType':
    'Select the web service you want to use to build integrations in Microsoft Dynamics 365 Business Central. You can also <a href="https://docs.microsoft.com/en-us/dynamics365/business-central/dev-itpro/webservices/web-services">learn more about Web services</a>.',
  'microsoftbuisnesscentral.connection.http.unencrypted.environmentName':
    "Enter your Microsoft Dynamics 365 Business Central's environment.<br>Steps to get the environment name:<br> 1. Log in to the Business Central Admin Center.<br>  2. Click on Environment available at the left side.<br>  3. Copy the Environment Name available on the list.",
  'precisely.http._iClientId':
    'Save your API key and secret of Precisely account in iClient for an added layer of security.',
...
if you're using a production account, you'll find your API keys under the 'API Management' page.<br>Multiple layers of protection, including AES 256 encryption, are in place to keep your API key safe. When editing this connection, you must re-enter this value each time; it is stored only when the connection is saved and never displayed as text.`,
  ...
  'gainsight.connection.http.gainsightInstanceURL':
    "Please enter the instance URI shown in your Gainsight CS account.For example, if https://test-domain.gainsightcloud.com/v1/ui/home is the URL after logging in,then the instance URI would be 'test-domain.gainsightcloud'.",
  'gainsight.connection.http.auth.token.token':
  ...
  'dotdigital.connection.http.auth.basic.username':
    'Please enter the username from API user credentials of your Dotdigital account.To know how to create the API user, please follow the link <a href=https://developer.dotdigital.com/docs/getting-started-with-the-api#setting-up-your-api-user>https://developer.dotdigital.com/docs/getting-started-with-the-api#setting-up-your-api-user</a>.',
  'dotdigital.connection.http.auth.basic.password':
  ...
  'channelape.connection.http.auth.token.token':
  ...
  'checkout.connection.environment':
    'Please select the environment of your Checkout.com account.',
  'checkout.connection.http.auth.token.token':
  ...
  'api.name':
    'Name your API so that you can easily reference it from other parts of the application.',
  'api.description':
    'Describe your API in more detail here so that other users can understand the problem you are solving, and also how your API works. Be sure to highlight any nuances that a user making changes in the future might want to know.',
  'api.shipworks.username':
    'Enter the username that was entered during the ShipWorks store setup.',
  'api.shipworks.password':
    'Enter the password that was entered during the ShipWorks store setup.',
  ...
  'asynchelper.http.submit.transform': `This field is optional. You can use it when the response is other JSON. For example, if your target application is Amazon, the response data received will be in XML format that you can convert to JSON using Transform Rules for Submit Response. For more information, refer to <a href="${HELP_CENTER_BASE_URL}/hc/en-us/articles/115002669511-Transformation-Rules-Guide">Transform Rules Guide</a>.`,
  'asynchelper.http.status._exportId': `Click the <b>+</b> icon to configure the status export. For more information, refer to <a href="${HELP_CENTER_BASE_URL}/hc/en-us/articles/360004872932-Async-Helper#h_73655244751529322925787">Configure Status Export</a>.`,
  ...
  'asynchelper.http.result._exportId': `Click the <b>+</b> icon to configure the result export. For more information, refer to <a href="${HELP_CENTER_BASE_URL}/hc/en-us/articles/360004872932-Async-Helper#h_15255507731529322901895">Configure Result Export.</a>`,
  'connection._id':
  ...
  'connection.http.headers':
    'In some rare cases, you may need to include custom HTTP headers with your API requests. <a href="https://www.celigo.com/ipaas-integration-platform/">integrator.io</a> automatically adds the appropriate content-type header based on the mediaType value described in the connection associated with this request (typically application/json). Note that the request header value automatically includes the authentication method described in the associated connection if required. Use this header field in the rare case when an API requires additional headers other than application or JSON.',
  ...
  'connection.http.auth.token.location':
    'This determines where your application’s API expects to find the authentication token.</br>Choose URL if the authentication token is located in the URL. You can then specify the query string parameter name that holds the token value.</br> Choose Header and specify the header name and authentication scheme to use when constructing the HTTP request. </br>Choose Body if your API requires the token to be embedded in the body structure of your HTTP request. In such cases, place the token in your body template using the placeholder: {connection.http.token.token}.',
  'connection.http.auth.oauth.headerName':
    'By default, integrator.io sends all authentication type info in the authorization HTTP header field. If the API you are connecting to requires a different HTTP header, use this field to provide an override.',
  'connection.http.auth.wsse.headerName':
    'Provide “header name”, where the hash token generated needs to be added in the request headers. By default, the name is going to be <b>X-WSSE</b>.',
  ...
  'connection.http.auth.token.paramName':
    'Use this field to specify the name of the URL parameter that holds the API token value. For example, if you specify myAPITokenURLParam, then all HTTP requests will include ?myAPITokenURLParam=[token]',
  ...
  'connection.http.encrypted':
    "Store all sensitive fields required by your imports and exports to access the app you are connecting to. For example, {'password':'celigorocks'} or {'token':'x7if4nkovhgr63ghp'}. These values are stored with AES-256 encryption and other layers of protection to keep your data safe.",
  'connection.http.unencrypted':
    "Store all non-sensitive fields required by your imports and exports to access the app you are connecting to. For example, {'email':'my_email@company.com', 'accountId': '8675301', 'role':'admin'}.",
  ...
  'snowflake.import.rdbms.queryType':
  ...
  'bigquery.import.rdbms.queryType':
    "'Use bulk insert SQL query' to quickly insert batches of data efficiently. </br>'Use SQL query once per page of records' to execute a custom query per page of records.</br>'Use SQL query on first page only' to execute a custom query that runs only once in a flow on the first page of records.",
  'redshift.import.rdbms.queryType':
    "'Use SQL query once per record' to execute a custom query per record. </br>'Use SQL query once per page of records' to execute a custom query per page of records.</br>'Use SQL query on first page only' to execute a custom query that runs only once in a flow on the first page of records.",
  ...
  'connection.rdbms.bigquery.projectId':
    'The unique identifier for the BigQuery project in the Google Cloud account.',
  'connection.rdbms.bigquery.clientEmail':
    'The email address for the Google Cloud service account used for authentication.',
  'connection.rdbms.bigquery.privateKey':
    'First, copy the private key from the Google portal for the service account that you want to use to authenticate the connection. Before you add it to integrator.io you must replace all newline characters (\\n) throughout the private key:<br>1. Paste the private key into a text editor.<br>2. Find \\n.<br>3. With your cursor in that location, delete the \\n characters and press Enter or Return.<br>4. Repeat this for each instance of \\n.<br>5. Ensure -----BEGIN PRIVATE KEY----- appears before the key, and -----END PRIVATE KEY----- appears after the key.<br>6. Copy and paste the reformatted private key (including the begin and end declarations) into integrator.io.',
  'connection.rdbms.bigquery.dataset':
    'The name of the dataset containing the tables and views being accessed.',
  'connection.rdbms.redshift.region':
    "Name of the amazon redshift region to the location where the request is being made. If not set, by default 'us-east-1' is selected",
  'connection.rdbms.redshift.aws.accessKeyId':
  ...
  'connection.rdbms.redshift.aws.secretAccessKey':
  ...
  'connection.rdbms.redshift.database':
    'redshift database that you want to connect.',
  'connection.rdbms.redshift.clusterIdentifier':
    'The name of the Redshift Cluster. <a href="https://docs.aws.amazon.com/redshift/latest/mgmt/configuring-connections.html" target="_blank">Learn more</a>.',
  ...
  'connection.dynamodb.aws.accessKeyId':
    'Enter the ID for this account set up by the DynamoDB server administrator.',
  'connection.dynamodb.aws.secretAccessKey':
    'Enter the access key for the account ID.',
  ...
  'connection.ftp.tradingPartner':
    'Set this flag to indicate that the FTP site represents an external trading partner. Doing so will ensure that this endpoint counts against the Trading Partners entitlement of your <a href="https://www.celigo.com/ipaas-integration-platform/">integrator.io</a> license.',
  ...
  'connection.pgp.passphrase': 'Enter the private key’s passphrase',
  ...
  'connection.ftp.concurrencyLevel':
    'Set this field to limit the number of concurrent/parallel requests allowed by the connection at any one time.',
  ...
  'connection.mode':
    'Select cloud if you are connecting to a publicly accessible application like Salesforce, Slack, etc... Select on-premise if you are connecting to an application that is behind a firewall like a database or custom HTTP endpoint.  Connecting to an on-premise application requires installing an agent behind your firewall.',
  'connector._id':
  ...
  'export.dynamodb.region':
    "Name of the DynamoDB region to the location where the request is being made. If not set, by default 'us-east-1' is selected.",
  ...
  'export.file.rowsPerRecord':
    'This setting has been deprecated in favor of the more powerful options in the <b>Sorting and grouping</b> section.',
  'export.file.groupByFields':
    'Select one or more fields to use for grouping records together.',
  'export.file.sortByFields':
    'Enter one or more fields to use for sorting records.',
  ...
  'export.file.decrypt':
    'Select the algorithm to decrypt files. Make sure you choose the same algorithm that is used to encrypt the files.',
  ...
  'export.file.csv.rowDelimiter':
    'The characters used to identify the end of a row.',
  ...
  'export.ftp.backupDirectoryPath':
    'Specify the directory path of the FTP folder where files will be backed up after successful transfer.',
  'export.unencrypted.apiType':
    '<b>Selling Partner API (SP-API)</b>: The Selling Partner API is a REST-based API and is an evolution of the legacy Amazon Marketplace Web Service (MWS) APIs. It’s recommended you integrate using SP-APIs.<br><b>Marketplace Web Service API (MWS)</b>: Amazon Marketplace Web Service (Amazon MWS) is the legacy web service API.',
  ...
  'flow.autoResolveMatchingTraceKeys':
    'Enable <b>Auto-resolve errors with matching trace key</b> to resolve other open errors with the same <a href="https://docs.celigo.com/hc/en-us/articles/360060740672" target="_blank">trace key</a> (unique field identifier).',
  'flow.manageAliases':
  ...
  'alias.aliasId':
    'Enter a descriptive, unique name to use as the Alias ID. This is what you will use in scripts to refer to the resource. Use only letters, numbers, hyphens(-) and/or underscores(_). For example, shopify_flow_neworders or netsuite-connection2_prod',
  'alias.description':
    'Enter a description that provides more context on how the resource is used in your integration.',
  'alias.resourceType':
    'Select the type of resource this alias will reference (such as a flow or import). Once you select the type, you can choose from a list of resources matching this type.',
  'alias.resource':
    'Select the specific resource for this alias to reference. For example, if you selected Connection as the Resource type and your integration has three connections, choose the one you want this alias to reference.',
  'iclient._id':
  ...
  'import.sampleData':
    'Enter a sample destination record here so that <a href="https://www.celigo.com/ipaas-integration-platform/">integrator.io</a> can help you later map fields between source and destination applications. The sample destination record should ideally contain all fields that you want to process in this flow, and should follow the exact same formatting rules that the destination application/API requires.',
  'import.distributed':
  ...
  'import.file.encrypt':
    'Select the algorithm to use to encrypt the files before transfer.',
  'import.file.pgp.symmetricKeyAlgorithm':
    'Select the symmetric key algorithm to use for encryption.',
  'import.file.pgp.hashAlgorithm':
    'The hashing algorithm to use when signing files. If this field is not set, then integrator.io will not sign files after encrypting them.',
  ...
  'import.http.body':
    'The field is used to define the exact HTTP request body that will get sent to the destination application endpoint.',
  'import.http.headers':
  ...
  'import.file.encoding':
    'The file encoding indicates how the individual characters in your data are represented on the file system. Leave this field blank if you do not know what the encoding format is. The default encoding is UTF-8 and the other supported format is: UTF-8, Windows-1252 and UTF-16LE',
  ...
  'import.file.batchSize':
    'Set this field to limit the number of files processed in a single batch request. Setting this field will not limit the total number of files you can process in a flow. This field allows you to optimize for really big files where bigger batches might experience network timeout errors vs. really small files where processing 1000 files in a single batch keeps the flow more performant. 1000 is the max value allowed.',
  'import.ftp.backupDirectoryPath':
    'Specify the directory path of the FTP folder where files will be backed up after successful transfer.',
  'import.ftp.XMLDocument':
    'This field is used to define the XML document that will get sent to the destination application endpoint.',
  ...
  'import.s3.backupBucket':
    'Specify the bucket name where files will be backed up after successful transfer.',
  ...
  'import.dynamodb.region':
    "Name of the DynamoDB region to the location where the request is being made. If not set, by default 'us-east-1' is selected.",
  'import.dynamodb.method':
    '<b>putItem: </b>Creates an item or replaces an old item with a new item.<br><b> updateItem: </b>Edits an existing item’s attributes or adds a new item to the table if it does not already exist.',
  ...
  'export.salesforce.distributed.requiredTrigger':
    'A Salesforce trigger is required per sObject type to facilitate real-time exports.  Please copy the base trigger code provided in this field, and then configure the trigger to run in your Salesforce account.  You can change the logic in the trigger however is needed to meet specific business requirements.',
  'export.uploadFile':
  ...
  'export.salesforce.distributed.referencedFields':
  ...
  'export.salesforce.distributed.relatedLists':
  ...
  'export.netsuite.api.type':
    "NetSuite supports two different API types: RESTlet and Web Services. The RESTlet API is recommended because it is newer, more user friendly, and much easier to get started with. It also supports greater levels of concurrency when using just a regular NetSuite license (i.e. a license that does not have NetSuite's SuiteCloud Plus enabled), and the RESTlet API enables more advanced capabilities like the ability to define a preSend hook in NetSuite using SuiteScript, or to support more performant and robust 'Once' type exports. The Web Services API is a good fit when you've purchased a NetSuite SuiteCloud Plus license and you also need to export very large amounts of data. The Web Services API is the only option available if you cannot install our integrator.io bundle in your NetSuite account. To completely maximize the amount of data that can be exported from a single NetSuite license you would use a combination of both RESTlet and Web Services API based exports.",
  'export.netsuite.distributed.useSS2Framework':
    'Select which NetSuite SuiteScript API version you’d like to configure your real-time export for. If selecting SuiteScript 1.0, you need to have Celigo’s integrator.io bundle installed in your NetSuite account. If selecting SuiteScript 2.0, you need to have installed Celigo’s integrator.io SuiteApp in your NetSuite account. In order to integrate with NetSuite, you must have one of these installed in your NetSuite account. Read more about the new SuiteSript 2.0 APIs <a href="https://docs.celigo.com/hc/en-us/articles/360047138512" target="_blank">here</a>.',
  'export.netsuite.restlet.useSS2Restlets':
    'Select which NetSuite SuiteScript API version you’d like to configure your export for. If selecting SuiteScript 1.0, you need to have Celigo’s integrator.io bundle installed in your NetSuite account. If selecting SuiteScript 2.0, you need to have installed Celigo’s integrator.io SuiteApp in your NetSuite account. In order to integrate with NetSuite, you must have one of these installed in your NetSuite account. Read more about the new SuiteSript 2.0 APIs <a href="https://docs.celigo.com/hc/en-us/articles/360047138512" target="_blank">here</a>.',
  'import.netsuite_da.useSS2Restlets':
    'Select which NetSuite SuiteScript API version you’d like to configure your import for. If selecting SuiteScript 1.0, you need to have Celigo’s integrator.io bundle installed in your NetSuite account. If selecting SuiteScript 2.0, you need to have installed Celigo’s integrator.io SuiteApp in your NetSuite account. In order to integrate with NetSuite, you must have one of these installed in your NetSuite account. Read more about the new SuiteSript 2.0 APIs <a href="https://docs.celigo.com/hc/en-us/articles/360047138512" target="_blank">here</a>',
  ...
  'import.netsuiteImportFieldMappingSettings': `The type of field mapping that you want to perform. For more information, refer to <a href="${HELP_CENTER_BASE_URL}/hc/en-us/articles/360019506771" target="_blank"/>Map source record fields to destination</a>.`,
  'import.etailImportFieldMappingSettings': `The type of field mapping that you want to perform. For more information, refer to <a href="${HELP_CENTER_BASE_URL}/hc/en-us/articles/360019506771" target="_blank"/>Map source record fields to destination</a>.`,
  ...
  'import.editMockInput':
    'Use this mock data to preview and send import requests. The data can be modified to test different import scenarios, or to help with import configurations. The mock input is stored temporarily for the duration of the session, and is used for testing purposes only.',
  ...
  'connection.http.auth.oauth.grantType':
    'OAuth 2.0 authentication currently supports two grant types: authorization code and client credentials. Choose Authorization code to use an authorization server to obtain an authorization code as an intermediary between the client and resource owner. Choose Client credentials if authorization scopes are limited to protected resources under the control of the client or previously configured on the authorization server.',
  'connection.http.auth.oauth.callbackURL':
    'Use the callback URL to exchange secure messages with the authorization server after authentication. You must whitelist this URL with your authorization server.',
  'connection.http.auth.oauth.clientCredentialsLocation':
    'Sends a basic auth request in the header or client credentials in the request body.',
  ...
  'connection.http.auth.oauth.refreshHeaders':
    'In some cases, it may be necessary to include custom HTTP headers with your token refresh requests. As with the body field, you can reference any value from the connection using {placeholders with a complete path matching the connection field.',
  ...
  'concur.connection.http.unencrypted.username':
    'Please enter the Username of your Concur account.',
  'concur.connection.http.encrypted.password':
    'Please enter the Password of your Concur account.',
  'concur.connection.http.unencrypted.consumerKey':
    'Please enter the consumer Key of your Concur account.',
  'concurall.connection.http.unencrypted.username':
    'Please enter the ID value which will be available in the URL after connecting to Concur App present at the App Center.',
  'concurall.connection.http.encrypted.password':
    'Please enter the Request token value which will be available in the URL after connecting to Concur App present at the App Center.',
  'concurall.connection.http.subdomain':
    'Please enter the subdomain of your Concur account.',
  'concurall.connection.http.unencrypted.clientId':
    'Please enter Client ID of your Concur account.',
  'concurall.connection.http.encrypted.clientSecret':
    'Please enter Client Secret of your Concur Account.<br>Multiple layers of protection, including AES 256 encryption, are in place to keep your Client secret safe. When editing this connection, you must re-enter this value each time; it is stored only when the connection is saved and never displayed as text.',
  'concurall.connection.http.unencrypted.credtype':
    'For connections from the App Center, use authtoken. if omitted, oauth2 will assume the type is password.',
  ...
  'connection.netsuite.wsdlVersion': `The NetSuite WSDL version that this connection uses. It's always recommended to use the latest WSDL version. You can switch between different versions by editing the connection.
...
<br/>Note that NetSuite WSDL 2020.2 and later versions only support token-based auth.`,
  ...
  'connection.as2.preventCanonicalization':
    '“Canonicalized” data has been transformed to its simplest essential form during HTTP requests. Check this box to skip canonicalization during signature verification, which avoids any risk of the signature’s getting invalidated when accompanying transformed binary data',
  ...
  'flow.endTime': 'Set the final time that you want your flow to run each day.',
  'flow.daysToRunOn':
  ...
  'accesstoken._apiIds':
    'Select the My API that this token should provide access to.',
  ...
  'mapping.v2.fieldMappingType':
    'Select the type of mapping you want to apply to the destination field:<br><b>• Standard</b>: Send the value of the source field as the value in the destination field.<br><b>• Hard-coded</b>: Set the source field to a defined value for the destination field.<br><b>• Handlebars expression</b>: Build a handlebars expression for the value in the source field to send to the destination field.<br><b>• Lookup</b>: Define a static value-to-value mapping or use the results of a dynamic search to populate the destination field value.<br>See <a href="https://docs.celigo.com/hc/en-us/articles/360019506771-Map-source-record-fields-to-destination">Map source fields to destination</a>.',
  'mapping.v2.useDate':
    'Check this box if the destination field is a date type field. Additional options display if this box is checked.',
  ...
  'mapping.v2.generateDateFormat':
    'Select the date format for the destination field',
  'mapping.v2.generateDateTimezone':
    'Select the timezone for the destination field',
  'mapping.v2.default': 'Enter a custom value to set as the default value.',
  'mapping.v2.expression':
    'Use the advanced field editor (AFE) to build a handlebars expression. The result of the handlebars expression will populate the destination field.',
  'mapping.v2.lookup.mode':
    "<b>• Static or value-to-value lookup</b>: Use a static lookup if you already know all possible source and destination field values and how the lookup data should be mapped. For example, if you are mapping shipping methods between two applications, use a static lookup to define the source and destination mappings. You can use handlebars expressions if necessary.<br><b>• Dynamic lookup</b>: Use a dynamic lookup to search for record data by sending an HTTP call to the import application's API. For example, if you have an email address in your source, you can use the email address to search for additional data from the associated records in the import application.",
  'import.v2.lookup.name':
    'Enter a descriptive and unique name to identify this lookup for future use.',
  'mapping.v2.relativeURI':
    "Enter the resource path portion of the API endpoint. For example: '/product' or '/bulkUpdate/orders'. The relative URI value is appended to the base URI defined in the connection resource associated with this import. The base URI and relative URI together complete a fully qualified URL that defines the API endpoint. You can sometimes use query string parameters to pass extended information to an API endpoint. See the lookup application's API documentation for available endpoints and query string parameters.",
  'mapping.v2.lookup.method':
    "Provide the HTTP method required by your lookup application to retrieve the lookup data (options include GET, or POST). Consult your destination application's API documentation to determine which method is best to use for your lookup.",
  'mapping.v2.lookup.extract':
    'Enter the JSON path to the lookup field in the response body returned by the lookup application. For example: "products[0].name".',
  'mapping.v2.lookup.body':
    'Define the exact HTTP request body that integrator.io will send to the destination application endpoint. Any mapping configurations you have configured with the user interface will be ignored, and only the HTTP request body text entered in this field will be sent in the request.',
  'mapping.v2.copyObject':
    "Select <b>Yes</b> if there is an object in the source that you want to send to the target application without modification. integrator.io will send exactly the same child fields and their values to the target application. Otherwise, select <b>No</b> to define the mappings for each child field of the object. You don't need to add any mappings for the object type of the destination field.",
  'mapping.v2.copyObjectArray':
    'Select <b>Yes</b> if there is an object in the source that you want to send to the target application without modification. integrator.io will send exactly the same child fields and their values to the target application. Otherwise, select <b>No</b> to define the mappings for each child field of the object.',
  'mapping.v2.objectAction':
    'Action to take if the mapping returns an empty value:<br><b>• Use null as default value</b>: The value will be set to "null".<br><b>• Do nothing</b>: The record will be skipped.',
  'mapping.v2.multifieldAction':
  ...
  'mapping.v2.staticLookupAction':
    "Action to take if the source field value does not match any field values in the lookup record<br><b>• Fail record</b>: Records without any match will fail without generating errors.<br><b>• Use empty string as default value</b>: The value will be set to an empty string ('').<br><b>• Use null as default value</b>: The value will be set to \"null\".<br><b>• Use custom default value</b>: The value will be set to the custom value you define. Only use TRUE or FALSE as the custom value as the destination field is a boolean data type.<br><b>• Do nothing</b>: The record will be skipped.",
  ...
  'mapping.useFirstRow': `Check this box to update the destination sublist fields using only the value of the first sublist retrieved for the record. For example, if the source app contains the following two rows:
...
`,
  ...
  'mapping.fieldMappingType': `The type of field mapping that you want to perform. For more information, refer to <a href="${HELP_CENTER_BASE_URL}/hc/en-us/articles/360019506771" target="_blank"/>Map source record fields to destination</a>.`,
  'mapping.extract':
    'This dropdown lists all the available fields from your export record that can be used in your expression. Either by themselves, or as argument value for any selected helper methods.',
  'mapping.expression':
    'This field represents your complete handlebars expression. You have the freedom to manually enter an expression, or use the function and field drop-downs above to help construct it.',
  ...
  'mapping.lookup.method':
    'Select the HTTP method to use when performing the lookup query.',
  'mapping.lookup.extract':
    'Enter the JSON path in the lookup response body where the expected lookup value, to be set in the import application, will be found, such as "products[0].name".',
  ...
  'lookup.transform':
    'Define one or more transformation rules to rename fields, remove fields, and/or structurally optimize records returned by the export before the records are passed to the next step in the flow.<br/>• At the left, select the field you want to extract from your export data. Then enter the field name to generate.<br/>• You must extract all of the fields you want to keep, even if you do not modify all of them.<br/>• Transformation rules are applied in the order in which they appear; drag and drop the rules to change the order.',
  'import.transform':
    'Define one or more transformation rules to rename fields, remove fields, and/or structurally optimize the response data returned by the import before the response data is merged back into the source record.<br/>• At the left, select the field you want to extract from the response data. Then, enter the field name to generate.<br/>• Transformation rules are applied in the order in which they appear; drag and drop the rules to change the order.',
  'users.user':
    'All users who have access to your account and integrations, or who have been invited to join your account.',
  'users.accesslevel':
    'Users with administer-level access can perform all the same actions as an account Owner, with the exception of transferring account ownership to another user, and will have full access to all integrations in the account. Users with manage level access can make changes to the integration. Users with monitor level access can view the integration only for the purpose of running flows and troubleshooting integration errors. Owner and Administrator users are responsible for provisioning access to the integration. <a href="https://docs.celigo.com/hc/en-us/articles/115003929872" target="_blank">Find out more about role and permissions.</a>',
  'users.status':
    "'Pending' means the user has not yet accepted the invite to your account.  'Accepted' means the user has accepted the invite to your account.  'Dismissed' means the user dismissed the invite to your account.",
  'users.enable':
    'This enables you to revoke access without deleting the user from the account. If Off, then the user will no longer be able to switch to this account - it will no longer show up in their <b>Accounts</b> drop-down.',
  'users.accountSSOLinked':
    ' This field indicates whether the user has successfully signed in using the account’s SSO settings. If yes, the user can only sign in to integrator.io using this account’s SSO settings. If no, the user can sign in to integrator.io using other authentication methods.',
  'users.requireAccountSSO':
    'This field indicates whether the user is required to sign in with the account’s SSO settings.',
  'users.actions':
    'These are actions the account owner can perform, like <b>Make account owner</b>, which will make that user the owner of the selected account. <b>Change permissions</b> enables the account owner to manage each user’s access level. <b>Delete</b> will delete the user from the account and they will no longer have access.',
  'accountdashboard.numRuns':
    'The number of times the flow has completed for the selected date range.',
  'accountdashboard.avgRuntime':
    'The average time the flow takes to complete running. The run time includes the time the flow is “Waiting in queue” status.',
  'accountdashboard.numResolvedByAuto':
    'The number of errors that were auto-resolved, based on a match to another error’s <a href="https://docs.celigo.com/hc/en-us/articles/360060740672" title="https://docs.celigo.com/hc/en-us/articles/360060740672" data-renderer-mark="true">trace key</a> (unique field identifier).',
  'runConsole.resolved':
  ...
  'runHistory.flow':
    'Expand a flow to see the historical breakdown of success, ignored, errors and pages encountered at each flow step (such as an export) from previous flow runs. The total number of errors, if any, reported at that flow step contains a link to complete details and actions for each error.',
  'runHistory.errors':
    'The error amounts (as well as the amounts in Success, Ignored and Pages columns) are historical dashboard data from previous flow runs. Resolving additional errors will not reduce the error amounts. The number of errors per step is the total of its open and resolved errors.',
  'myaccount.name':
    'This field will be displayed to other integrator.io users that you are collaborating with, and is also used by Celigo to administrate your account/subscription.',
  'myaccount.email':
    'Email is a very important field. Password reset links, product notifications, subscription notifications, etc. are all sent via email.It is highly recommended that you take the time to secure your email, especially if you are integrating sensitive information with your integrator.io account.',
  ...
  'myaccount.developer':
    "Turning on this setting will expose developer centric fields in the integrator.io UI. For example, when defining an 'Export' or an 'Import' there are 'Hooks' fields available in the UI where custom code can be configured.",
  'myaccount.showRelativeDateTime':
    'Check this box to view the amount of time elapsed since an event occurred, such as <b>1 hour ago</b> or <b>1 day ago</b>. Otherwise, the display format is absolute, such as <b>2021-10-25 7:47:33 PM</b>.',
  ...
  parentSObjectType:
    'Use this field to specify the additional referencelist (child sObjects of parent sObject) that you would like to add to the export data. Ex: Contact is a childSObjectType for the parentSObject type Account.',
  'pageGenerator.application':
    'Choose an application, database, or universal connector.',
  'pageGenerator.type':
    'The source options presented – or automatically selected for you – depend on the features available in the application that you chose.',
  'pageGenerator.connection':
    'Choose an existing connection to this application, or click the <b>+</b> icon to create a new connection. You can always change your connection later.',
  'pageGenerator.exportId':
    'Select an existing export, listener, or transfer that you have already created in your account for this application, in order to add that same resource to the current flow. To set up a new resource, leave this field blank and click <b>Next</b>.',
  'pageProcessor.application':
    'Choose an application, database, or universal connector.',
  'pageProcessor.connection':
    'Choose an existing connection to this application, or click the <b>+</b> icon to create a new connection. You can always change your connection later.',
  'pageProcessor.resourceType':
    'The destination options presented – or automatically selected for you – depend on the features available in the application that you chose.',
  'pageProcessor.importId':
    'Select an existing import, lookup, or transfer that you have already created in your account for this application, in order to add that same resource to the current flow. To set up a new resource, leave this field blank and click <b>Next</b>.',
  'pageProcessor.exportId':
    'Select an existing import, lookup, or transfer that you have already created in your account for this application, in order to add that same resource to the current flow. To set up a new resource, leave this field blank and click <b>Next</b>.',
  referencedFields:
    'Use this setting to add additional fields on the childSObject to the export data defined as lookup fields on Salesforce. Ex: If Contact is set as the childSObjectType, this setting allows users to pull data from the reference fields (such as Created By, Account Name) on the Contact sObject.',
  childRelationship:
    'Use this field to specify the additional referencelist (child sObjects of parent sObject) that you would like to add to the export data. Ex: Contact is a childSObjectType for the parentSObject type Account.',
  'salesforce.referencedFields':
    'Use this setting to add additional fields on the childSObject to the export data defined as lookup fields on Salesforce. Ex: If Contact is set as the childSObjectType, this setting allows users to pull data from the reference fields (such as Created By, Account Name) on the Contact sObject.',
  filterExpression:
    'Use this field to filter out any reference list by entering the "where" clause of SOQL query. This expression will be added as a part of the SOQL query in the where clause while fetching the childSObjectType. If no filter is added, IO will send all the child SObjects in the export data. Ex: If you would like to only export Contacts whose LastName has "Bond" in it, set the expression as "LastName=`Bond` ".',
  orderBy:
    'Use this field to specify how you would like to order the related list records. Ex: Use `CreatedDate` to order the records by date created. The default order is `Ascending order`. To change it to use descending order using the order by field as `CreatedDate DESC`.',
  'afe.sampleDataSwitch':
    "Advanced Field Editor (AFE) 2.0 exposes an improved set of context variables, thus making it much easier to build dynamic values and/or complex structures. More specifically, the generic 'data' variable has been replaced by a set of more specific variable names to better represent the structure of data your template must work with. For example: 'record', 'rows[]', 'batch_of_records[]', etc…",
  'afe.mappings.toggle':
  ...
  'afe.mappings.v2.rule':
    'Define the JSON structure for the data to send to the target application. Get more details <a href="https://docs.celigo.com/hc/en-us/articles/4536629083035-Mapper-2-0">here</a>',
  'afe.mappings.v2.input':
    'This is an example of the source application\'s input data received by the mapping for this flow step. You can also view the data classification (<a href="https://docs.celigo.com/hc/en-us/articles/4409527888923-Work-with-JSON-data-in-integrator-io#JSON-record-or-row">record or row</a>) in this panel.',
  'afe.mappings.v2.output':
    'This is an example of the output data generated by your current mapping configuration. You can also view the data classification (<a href="https://docs.celigo.com/hc/en-us/articles/4409527888923-Work-with-JSON-data-in-integrator-io#JSON-record-or-row">record or row</a>) in this panel.',
  'afe.mappings.input':
    'The input area shows the complete source data as it enters the mapping processor.',
  'afe.filters':
    '<a href="https://docs.celigo.com/hc/en-us/articles/360025927811-Apply-filters">Applying filters</a> helps you refine your data as it progresses through a flow. Output filters refine the data retrieved from your source application, while input filters selectively accept incoming data for a lookup or import.',
  'afe.import.mapping':
  ...
  'afe.responseMappings.input':
    'The input area shows the parsed response from the import application, and this data is available to merge with existing source record data.',
  'afe.lookupMappings.input':
    'The Input area shows the parsed results from the lookup application, and this data is available to merge with existing source record data.',
  'afe.mappings.output':
    'The output area shows the complete data set that will be sent to the import application, after the mapping has been applied.',
  'afe.responseMappings.output':
    'The output area shows the new source record data after the import response has been added or merged.',
  'afe.lookupMappings.output':
    'The output area shows the new source record data after the lookup data has been added or merged.',
  'afe.mappings.netsuite.assistant':
    'The NetSuite Mapping Assistant emulates your NetSuite account, displaying a standard form with only the visible fields. Click a destination field to be mapped, and then select the field in the Assistant to populate your import mapping with the NetSuite (path and) field name.',
  'afe.mappings.salesforce.assistant':
    'The Salesforce Mapping Assistant emulates your Salesforce account, displaying a standard form with only the visible fields. Click a destination field to be mapped, and then select the field in the Assistant to populate your import mapping with the Salesforce (path and) field name.',
  'mapping.suitescript.netsuite.lookup.searchField':
    'Field name that is provided to the lookups defined, using this field name the respective field from the record will be extracted out and provided for the respective field in the import system.',
  'mapping.suitescript.lookup.failWhenUniqueMatchNotFound':
    'When this flag is set, if no results are found or the dynamic lookup fails, the lookup will silently fail(return empty string). Similarly, if multiple results are found (dynamic lookup) then the first value is chosen. In other words, if allowFailures is set to true, then no errors will be raised and the default lookup value will be used if the lookup fails.',
  'mapping.lookup.default':
    'This holds the default value to be set for the extract field.',
  'mapping.suitescript.lookup.useNull':
  ...
  'mapping.suitescript.lookup.useEmptyString':
...
use the custom option.`,
  'parser.xml.trimSpaces':
    'If checked, values will be stripped of leading and trailing whitespace.',
  'parser.xml.listNodes':
    "It is not always possible to infer which XML nodes are single values or a list. To force an XML node to be recognized as a list (Array), enter it's path here.",
  'parser.xml.includeNodes':
    'Often XML documents are large and their full content is not needed. It is possibly to reduce the record size by specifying only the set of nodes (specified by path) that should be extracted.',
  'parser.xml.excludeNodes':
    'It may be easier to specify node to exclude than which to include. If you wish to exclude certain xml nodes from the final record, specify them here using a simplified xpath.',
  'fb.pp.inputFilter':
  ...
  'sso.orgId':
    'Enter a unique organization ID that integrator.io will use to generate a unique SSO URL for your account. This field only accepts alphanumeric characters and must be 3-20 characters in length.',
  'sso.loginURL':
    'Use this unique URL to sign in to your account with SSO. Use this URL  when configuring the integrator.io app in your SSO provider.',
  'sso.redirectURL':
    'Your SSO provider will send the authorization code tokens to this unique URL. Use this URL when configuring the integrator.io app in your SSO provider.',
  'sso.primaryAccount':
    'Choose the account that you would like to use for SSO. Every time you sign in via SSO, integrator.io will verify that the SSO provider is linked to this specific account.',
  classification:
    'Errors are automatically categorized according to their properties, such as the code, message, and source fields. For more information, see <a href="https://docs.celigo.com/hc/en-us/articles/4403697564429" target="_blank">Error classifications</a>',
  'import.lookup':
    'Choose an existing lookup from the list or click <b>+</b> to define a new lookup. Lookups provide a way to dynamically search the destination application on the fly to find existing records.',
  'import.lookupType':
    'Choose the method that should be used to identify if a source record already exists in the destination application: <br><b>• Records have a specific field populated – </b> <br>This method checks if a specific field in the source record already has a value, and then if so, the source record will be considered an existing record.<br>  <b>• Run a dynamic lookup – </b>This method will run a search in the destination application to find a record on the fly, and then if a record is found, the source record will be considered an existing record.',
  'import.ignoreExtract':
    'Enter the path to the field in the source record that should be used to identify existing records. If a value is found for this field, then the source record will be considered an existing record.',
  'import.existingExtract':
    'Enter the path to the field in the source record that should be used to identify existing records. If a value is found for this field, then the source record will be considered an existing record.',
  'import.existingLookupName':
    'Choose an existing lookup from the list or click <b>+</b> to define a new lookup. Lookups provide a way to dynamically search the destination application on the fly to find existing records.',
  ...
  'import.netsuite.file.internalId':
    'Enter a reference ID or expression to identify reference IDs of files to replace in the NetSuite File Cabinet.',
  'auditlogs.download':
    'Download up to 1,000 records from the last year. Note that filters are not applied to a download.',
  'revisions.pull':
    'Pulling changes from one linked integration to another lets you see conflicts, review changes, and merge them from one integration to another. You can only pull data from a direct clone or source integration.. For example, clone Integration A as Integration B, then B to C, and B to D. You can create a pull between integrations A and B, B and C, B and D; but not between A and C, or C and D',
  'pull.integration':
    'Select the remote production or sandbox integration from which you pull changes. All linked integrations are displayed, which includes clones of this integration and the integration from which it was cloned.',
  'pull.reviewChanges':
    'Review the changes between the current and remote integration. You can see all your integration resource changes, except for sensitive resources like API tokens and connections. You must resolve conflicts before you can continue to merge your changes. You’ll configure any new connections in the Merge changes step.',
  'pull.mergeChanges':
    'You must first configure any new connections that are coming from the remote integration. There may be additional installations steps depending on the type of connection, including installing remote packages. You can cancel your merge at any time, or close the window and resume your merge later.',
  'revert.create':
    'When creating a revert, you are restoring to a revision of an integration. The revert action will also generate a revision',
  'revert.description': 'Enter information about the revert',
  'revert.revision.createdAt':
    'The date and time of the revision you’ll revert to.',
  'revert.revisionId': 'The revision you are reverting to.',
  'revert.reviewChanges':
    'Review the changes between your current revision and the revision that you are reverting to. This is a list of every change you’ve made to your exports, imports, flows, and scripts.',
  'revert.finalRevertChanges':
    ' You can make changes to a source or cloned integration, then revert them. You must reconfigure your connections to revert your changes and there may be additional revert steps depending on necessary packages',
  'snapshot.create':
    'A snapshot is a saved capture of your integration that you can use to revert your integration at any point.',
  'snapshot.description':
    'Describe your snapshot so you can quickly identify the revision and any important details.',
  'mfa.enable':
  ...
  'http.parseSuccessResponses':
    'The integrator.io works on JSON internally. If your API returns success data in a different format than JSON, parse HTTP response data using available parsers for CSV and XML.',
};
