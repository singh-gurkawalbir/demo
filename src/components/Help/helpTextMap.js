...
export default {
 ...
 <br>Multiple layers of protection, including AES 256 encryption, are in place to keep your API key safe. When editing this connection, you must re-enter this value each time; it is stored only when the connection is saved and never displayed as text.`,
  ...
  'listener.debugLogs': 'Click Start debug to debug your resource. Select a duration, then run your flow to capture the logs. Click Refresh logs to see the latest logs. <br> To view logs, click on a timestamp in the Time column. On the right, you can see the request made by integrator.io to the flow step application at that time, and the response returned by the application. The Parsed output tab will show the data returned by the flow step application for this request.',
