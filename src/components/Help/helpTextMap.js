...
export default {
  ...
  'pagerdutyevents.connection.http.encrypted.integrationKey': 'Enter the Integration Key. This is the GUID of one of your Events API V2 integrations.<br> <b>Steps to get the key:</b><br>1. Log in to the Pagerduty account.<br>2. From the dashboard, Navigate to <b>Integrations</b> → <b>Developer Tools</b> → <b>Developer Mode</b>.<br>3. Select an App. Under functionality, Click the manage option for Events Integration.<br>4. From the Event API endpoint, copy the Integration Key. For example, if the endpoint looks like “https://events.pagerduty.com/integration/Integartion-Key/enqueue”. Copy the value which is in place of <b>Integration-Key</b> to authenticate the connection.<br><br>Multiple layers of protection, including AES 256 encryption, are in place to keep your key safe. When editing this connection, you must re-enter this value each time; it is stored only when the connection is saved and never displayed as text.',
