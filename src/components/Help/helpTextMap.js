...
export default {
 ...
 Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your API key safe.`,
  ...
  'googleads.connection.http.encrypted.developerToken': 'Please enter the Developer token.<br>The Google Ads API also requires a developer token in order to make calls to the API. You can apply for a token for your Manager Account directly from the Google Ads UI.<br>Please follow these steps to fetch the developer token:<br> 1. To retrieve your developer token, sign in to your Manager Account.<br> 2.Navigate to <b>TOOLS > MORE TOOLS > API Center</b>. The API Center option will appear only for Google Ads Manager Accounts.<br> 3.You will find your <b>Developer token</b> under <b>API Access.</b> <br><br>Multiple layers of protection, including AES 256 encryption, are in place to keep your token safe. When editing this connection, you must re-enter this value each time; it is stored only when the connection is saved and never displayed as text.',
  'googleads.connection.http.unencrypted.customerId': 'Please enter the Customer ID.<br>Steps to fetch the Customer ID:<br> 1. Sign in to your Google Ads account.<br> 2. Click the help icon <b>?</b> on the top right corner. <br> 3.Find <b>Customer ID</b> at the bottom of the menu. <br> If your using a client account to make API calls, enter the client customer ID in the <b>Customer ID</b> field and manager customer ID in the <b>Login customer ID</b> field.',
