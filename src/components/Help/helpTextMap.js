...
const extractedHelpkeys = {
  'connection.as2.partnerStationInfo.mdn.verifyMDNSignature':
    'regex connections regex Check this box if your trading partner requires that the MDN signature be verified. Otherwise, integrator.io will not attempt to verify the signature.',
  'connection.as2.userStationInfo.mdn.mdnURL':
    "regex connections regex This is the URL via which integrator.io will send asynchronous MDNs to your trading partner. Note that this URL will typically be different to the Partner's AS2 URL field above.",
  'connection.partnerrequireasynchronousmdns':
    'regex connections regex Check this box if your trading partner requires MDNs to be sent asynchronously. By default, integrator.io is configured to send MDNs synchronously.',
  'export.netsuite.distributed.executionContext':
    'regex exports regex The invited user will have permissions to manage the integrations selected here.',
  'export.netsuite.distributed.executionType':
    'regex exports regex The invited user will have permissions to manage the integrations selected here.',
  'iClient.oauth2.clientId':
    'regex iClients regex This is the ID for your client app that is registered with the API provider.',
  'iClient.oauth2.clientSecret':
    'regex iClients regex This is the client secret the API provider gave you.',
  'import.http.requestType':
    'regex imports regex Please specify whether the record is being created or updated using this field.',
  'import.rdbms.queryType':
    "regex imports regex Please select 'Insert' if you are only importing new records into the Database. Please select 'Update' if you are only importing changes to existing records in the Database. Please select 'Insert or Update' if you want your import to be more dynamic such that (1) if an existing record exists in the Database then that record will be updated, or (2) if an existing record does not exist then a new record will be created.",
  'import.rdbms.ignoreExtract': `regex imports regex This field is used to inform integrator.io on how to identify existing records, and if a record is found to exist, it will be ignored (no operation performed for this record). integrator.io will determine if a record exists by the presence of a specific record property. Typically this would be a field that is only present on existing records such as an "ID", or "createDate". If this is the case, simply provide the field path to this property. Example: "customerId" or "dateCreated".
  ...
  'import.rdbms.updateExtract': `regex imports regex This field is used to inform integrator.io on how to identify existing records, and if a record is found to not exist, it will be ignored (no operation performed for this record). integrator.io will determine if a record exists by the presence of a specific record property. Typically this would be a field that is only present on existing records such as an "ID", or "createDate". If this is your case, simply provide the field path to this property. Example: "customerId" or "dateCreated".
    Alternatively you can identify existing records by using the result of a lookup. If the lookup returned a value, then this would be an indication that the record exists. An example of this would be a lookup that maps an email from the export record to an ID from the destination App. If this is how you wish to identify an existing lookup, first define the lookup and then simply enter the lookup's name in this field.`,
  'import.rest.requestType':
    'regex imports regex Please specify whether the record is being created or updated using this field.',

  'connection.customScopeDelimiter':
    'regex http regex If your provider does not use spaces to delimit scopes, check this box, then provide the custom scope delimiter for your provider.',
  'connection.salesforce.info.email':
    'regex salesforce regex Your Salesforce account email.',
  'connection.salesforce.info.organization_id':
    "regex salesforce regex Your organization's unique Salesforce ID",
  'connection.rest.bearerToken':
    'regex xcart regex Please enter your token here. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your Token safe. This can be obtained by navigating to Tokens page from the options menu on the top right corner in the application.',
  'connection.http.subDomain':
    "regex sharepoint regex Please enter your SharePoint subdomain. For example, in https://temp-portal.sharepoint.com 'temp-portal' is the subdomain.",
  'connection.accountType':
    'regex vroozi regex Please select your account type here. Select Sandbox if your API Endpoint starts with https://sandbox-api.vroozi.com. Select Production if your API Endpoint starts with https://api.vroozi.com',
  'connection.http.unencrypted.apiKey':
    'regex vroozi regex Please enter your API Key here.API key is generated after creating your application.',
  'connection.http.auth.token.token': `regex vroozi regex Please enter your Access Token here. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your API key safe. To get an API key for your Vroozi account,login to your Vroozi account and Under API Integration section, click on Credentials.Create your application by clicking on "Add New Application" After entering your application name, you will be provided with an access token which is shown only once.`,
  'connection.http.auth.token.token': `regex saplitmos regex Please enter API Key of your SAP Litmos Account.Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your user secret safe.Steps to generate API Key: Login to SAP Litmos Account -- > Click "My Profile & Settings" from the drop-down menu at the top right corner of the screen -- > View the bottom of your profile.`,
  'connection.accountType': `regex oandav20fxtrade regex Please select your account type here. Select Demo if your account type is "fxTrade Practice". Select Trading if your account type is "fxTrade".`,
  'connection.http.auth.token.token':
    'regex oandav20fxtrade regex Please enter your API token here.',
  'connection.environment':
    'regex openair regex Please select the environment of your OpenAir account.',
  'connection.http.unencrypted.companyId':
    'regex openair regex Please enter Company ID of your account.',
  'connection.http.unencrypted.userId':
    'regex openair regex Please enter User ID of your account.',
  'connection.http.encrypted.password':
    'regex openair regex Please enter Password of your account. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your Password safe.',
  'connection.http.unencrypted.namespace':
    'regex openair regex Please enter the API Namespace of your account.',
  'connection.http.unencrypted.apiKey':
    'regex openair regex Please enter the API Key of your account. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your API key safe.',
  'connection.http.oneloginRegion':
    'regex onelogin regex Please enter Region for URI.',
  'connection.http.unencrypted.apiKey':
    'regex onelogin regex Please enter API Key of your OneLogin Account.',
  'connection.http.encrypted.apiSecret':
    'regex onelogin regex Please enter API Secret of your OneLogin Account. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your user secret safe.',
  'connection.http.auth.token.token':
    'regex onelogin regex The access token of your OneLogin account.',
  'connection.http.unencrypted.publicKey':
    'regex paycor regex Please enter your public key here. Your public key identifies you to our system. This is similar to a username. You will include your public key every time you send request to Paycor. This is not secret information.',
  'connection.http.encrypted.secretKey':
    'regex paycor regex Please enter your private key here. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your private key safe. The private key is secret and is similar to a password. Only you and Paycor should have your private key. The shared secret allows access to your sensitive data.',
  'connection.http.quickbooksEnvironment':
    'regex quickbooks regex Please select your environment here. Select Sandbox Accounting if the account is created on https://sandbox-quickbooks.api.intuit.com. Select Sandbox Payment if the account is created on https://sandbox.api.intuit.com. Select Production Accounting if the account is created on https://quickbooks.api.intuit.com. Select Production Payment if the account is created on https://api.intuit.com.',
  'connection.http.unencrypted.companyId':
    'regex trinet regex Please reach out to TriNet support team for company Id.',
  'connection.http.auth.token.token':
    'regex trinet regex Please reach out to TriNet support team for API key. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your API key safe.',
  'connection.http.unencrypted.pacejetLocation':
    'regex pacejet regex Please reach out to Pacejet support team for location header.',
  'connection.http.encrypted.pacejetLicenseKey':
    'regex pacejet regex Please reach out to Pacejet support team for License key. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your License key safe.',
  'connection.integrator.environment':
    'regex integratorio regex Please select your environment here. Select Sandbox if the account is created on https://staging.integrator.io. Select Production if the account is created on https://integrator.io.',
  'connection.integrator.region':
    'regex integratorio regex Please select your region here. Select North America if the account is created on https://api.integrator.io. Select Europe if the account is created on https://api.eu.integrator.io.',
  'connection.integrator.token':
    'regex integratorio regex Please enter your token here. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your Token safe. This can be obtained by navigating to Tokens page from the options menu on the top right corner in the application.',
  'connection.http.auth.token.token':
    'regex recharge regex Please enter your API key here. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your API key safe. Access to the API will need to be given by a member of the ReCharge team so reach out to their support team to enable this for you. Once this has been enabled for your store, you can go to Integrations and click on API tokens on the far right corner of your dashboard.',
  'connection.http.bamboohrSubdomain':
    'regex bamboohr regex Please enter your company name here which you configured while signing up for a new BambooHR account.',
  'connection.http.encrypted.apiKey': `regex bamboohr regex Please enter your API key here. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your API key safe. To generate an API key for a given user, users should log in and click their name in the upper right hand corner of any page to get to the user context menu. There will be an "API Keys" option in that menu to go to the page.`,
  'connection.http.oktaSubdomain':
    'regex okta regex Please enter your Okta subdomain here which you configured while signing up for a new Okta account.',
  'connection.http.auth.token.token': `regex okta regex Please enter your Token here. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your API key safe. To generate an API key for a given user, users should log in and click their name in the upper right hand corner of any page to get to the user context menu. There will be an "API Keys" option in that menu to go to the page.`,
  'connection.http.auth.token.token':
    'regex easyship regex Please enter your API key here. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your API key safe. You can generate the API Access Token from https://app.easyship.com/connect. You will need to create an API connection, and then retrieve the token from the store settings.',
  'connection.environment':
    'regex propack regex Please select your environment here. Select Test if the account is created on https://test.webservices.p3pl.com. Select Production if the account is created on https://webservices.p3pl.com.',
  'connection.http.unencrypted.p3plUserID':
    'regex propack regex Please enter your P3PL account User ID.',
  'connection.http.encrypted.p3plUserPassword':
    'regex propack regex Please enter your P3PL account Password.',
  'connection.http.encrypted.securityKey':
    'regex solidcommerce regex Enter your Solid Commerce Developer Key or Security Key here. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your Security Key safe. This can be obtained from the Settings section and Security Key subsection.',
  'connection.http.encrypted.appKey':
    'regex solidcommerce regex Application Key is generated at https://www.upsefulfillment.com, under Marketplaces --> Marketplaces Setup --> Web Services.',
  'connection.http.encrypted.apiKey':
    'regex parseur regex Please enter your API key here. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your API key safe.',
  'connection.http.accountType':
    'regex paypal regex Select one of the following, depending on the account you’re connecting to. <br> <b>.</b> Sandbox </br> <b>.</b> Production',
  'connection.http.unencrypted.clientId':
    'regex paypal regex Please enter Client ID of your Paypal Account.Steps to generate API credentials: Login to Developer Account -- > My Apps & Credentials -- > Select the Sandbox or Live -- > Create an App -- > Copy the Client ID and Secret',
  'connection.http.encrypted.clientSecret':
    'regex paypal regex Please enter Client Secret of your Paypal Account. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your user secret safe.Steps to generate API credentials: Login to Developer Account -- > My Apps & Credentials -- > Select the Sandbox or Live -- > Create an App -- > Copy the Client ID and Secret',
  'connection.http.auth.token.token':
    'regex paypal regex The access token of your Paypal account.',
  'connection.http.encrypted.apiKey':
    'regex returnly regex Please enter your API key here. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your API key safe. This can be obtained from the Summary tab in Your Account section.',
  'connection.http.subdomain':
    'regex 4castplus regex The subdomain is the unique portion of the 4castplus.com address that you visit to log in to your portal.',
  'connection.http.unencrypted.username':
    'regex 4castplus regex Username is your 4castplus account email.',
  'connection.http.encrypted.password':
    'regex 4castplus regex Password is your 4castplus account password. Multiple layers of protection are employed (including AES 256 encryption) to keep your password safe.',
  'connection.http.unencrypted.partnerUserId':
    'regex expensify regex Please enter your partner user id of expenisfy account which can be obtained from  https://www.expensify.com/tools/integrations/ after creating Expensify account at https://www.expensify.com/.',
  'connection.http.encrypted.partnerUserSecret':
    'regex expensify regex Please enter your partner user secret of expenisfy account which can be obtained from  https://www.expensify.com/tools/integrations/ after creating Expensify account at https://www.expensify.com/. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your user secret safe.',
  'connection.http.encrypted.apiKey':
    'regex klaviyo regex Please enter your API key here. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your API key safe. This can be obtained from the Settings section and API Keys subsection.',
  'connection.http.unencrypted.organizationId':
    'regex zohodesk regex Please enter your organization id here. This can be obtained by using the Get All Organizations API. In Zoho Desk, each business is categorized as an organization. All APIs except the ones directly related to organizations must include the organization ID in the header.',
  'connection.http.auth.token.token':
    'regex tophatter regex Please enter your token here. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your Token safe. This can be obtained by navigating to Tokens page from the options menu on the top right corner in the application.',
  'connection.http.zohoSubdomain': `regex zohocrm regex Please enter the Domain. Input ".com" to connect to US data-center. Input ".in" to connect to India data-center. Input ".com.cn" to connect to China data-center. Input ".eu" to connect to Europe data-center.`,
  'connection.http.tsheetsSubdomain':
    'regex tsheets regex Please enter your subdomain here which can be obtained from the base url of your Tsheets account.',
  'connection.http.auth.type':
    'regex squareup regex Please select Authentication Type',
  'connection.http.auth.token.token':
    'regex squareup regex Enter your access token for Square here. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your API token safe.',
  'connection.http.encrypted.serverToken':
    'regex postmark regex Please enter your Server Token here. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your Server Token safe. Used for requests that require server level privileges. This token can be found on the Credentials tab under your Postmark server.',
  'connection.http.encrypted.accountToken':
    'regex postmark regex Please enter your Account Token here. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your Account Token safe. Used for requests that require account level privileges. This token is only accessible by the account owner, and can be found on the API Tokens tab of your Postmark account.',
  'connection.http.auth.type':
    'regex zendesk regex Please select Authentication Type',
  'connection.http.zendeskSubdomain':
    'regex zendesk regex Please enter your team name here which you configured while signing up for a new Zendesk account.',
  'connection.http.apiToken':
    'regex zendesk regex API tokens are managed in the Support admin interface at Admin > Channels > API',
  'connection.http.activecampaignSubdomain':
    'regex activecampaign regex Please enter your account subdomain here.',
  'connection.http.auth.token.token':
    'regex activecampaign regex Please enter your API key here. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your API key safe. This can be obtained from the Settings section and Developer subsection.',
  'connection.http.auth.token.token':
    'regex drift regex Please enter your access token here. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your access token safe. This token is automatically generated when you installed the app to your team. You can use this to authenticate your app.',
  'connection.authorizeNet.accType':
    'regex authorize.net regex Please select your account type here. Select Sandbox if your API Endpoint starts with https://apitest.authorize.net. Select Production if your API Endpoint starts with https://api.authorize.net.',
  'connection.http.encrypted.apiLoginID':
    'regex authorize.net regex Merchant’s unique API Login ID. The API Login ID is provided in the Merchant Interface and must be stored securely. The API Login ID and Transaction Key together provide the merchant authentication required for access to the payment gateway.',
  'connection.http.encrypted.transactionKey':
    'regex authorize.net regex Merchant’s unique Transaction Key. The merchant Transaction Key is provided in the Merchant Interface and must be stored securely. The API Login ID and Transaction Key together provide the merchant authentication required for access to the payment gateway.',
  'connection.recurlySubdomain':
    'regex recurly regex Please enter your subdomain here which you configured while activating your new Recurly account.',
  'connection.http.auth.basic.username':
    'regex recurly regex Please enter your API key here. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your API key safe. You can go to Integrations >> API Credentials to find it.',
  'connection.http.auth.basic.username':
    'regex cartrover regex Please enter your API User. Navigate to Merchant view on left hand side and click on API keys section to find API User.',
  'connection.http.auth.basic.password':
    'regex cartrover regex Please enter your API key here. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your API key safe. Navigate to Merchant view on left hand side and click on API keys section to find API Key.',
  'connection.http.unencrypted.sellerId': `regex amazonmws regex The account ID for the Amazon seller account you are integrating with. You do not need to include it in your relativeURI; integrator.io will automatically add it to all request parameters. If you do not know this value you can find it in the "Settings" section in Amazon Seller Central.`,
  'connection.http.unencrypted.mwsAuthToken':
    'regex amazonmws regex The MWS authorization token.',
  'connection.http.unencrypted.marketplaceId': `regex amazonmws regex Please specify the Amazon MWS "MarketplaceId" for this connection. This value is required for specific Amzaon MWS requests to succeed. Please note that you must be registered to sell in the Amazon MWS "MarketplaceId" selected, else your Amazon MWS calls will fail.`,
  'connection.http.unencrypted.marketplaceRegion':
    'regex amazonmws regex Please specify the Amazon MWS Region for this connection. Please note that you must be registered to sell in the Amazon MWS Region selected, else your Amazon MWS calls will fail.',
  'connection.http._iClientId': `regex amazonmws regex Please specify the Amazon MWS "MarketplaceId" for this connection. This value is required for specific Amzaon MWS requests to succeed. Please note that you must be registered to sell in the Amazon MWS "MarketplaceId" selected, else your Amazon MWS calls will fail.`,
  'connection.http.auth.token.token':
    'regex autopilot regex Please enter your API key here. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your API key safe. To get an API key for your Autopilot account, login to your Autopilot account and go to Settings and to Autopilot API.',
  'connection.environment':
    'regex ebay-xml regex Select either Production or Sandbox.',
  'connection.http.unencrypted.apiSiteId':
    'regex ebay-xml regex After you have specified the API Site ID, click Save & Authorize that opens up the eBay window where you can enter email/username and password to establish the connection with eBay.',
  'connection.http.threedcartSecureUrl':
    "regex 3dcart regex 3dcart merchant's Secure URL.",
  'connection.http.encrypted.PrivateKey':
    "regex 3dcart regex Your application's private key.Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your Private Key safe. This can be obtained from the Settings section and Private Key subsection.",
  'connection.http.auth.token.token':
    "regex 3dcart regex The 3dcart merchant's token.",
  'connection.http.subdomain':
    "regex snapfulfil regex Enter your Snapfulfil subdomain. For example, in https://syndemo-eapi.snapfulfil.net/ 'syndemo-eapi' is the subdomain.",
  'connection.http.auth.basic.username':
    "regex snapfulfil regex Please enter your snapfulfil account's username",
  'connection.http.auth.basic.password':
    "regex snapfulfil regex Please enter your snapfulfil account's password",
  'connection.http.auth.basic.username':
    'regex anaplan regex The username of your Anaplan account.',
  'connection.http.auth.basic.password':
    'regex anaplan regex The password of your Anaplan account.',
  'connection.http.namelyCompanyName':
    'regex namely regex Your subdomain. For example, https://mysubdomain.namely.com',
  'connection.http.auth.token.token':
    'regex namely regex The personal access token of your account on namely. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your Personal Access Token safe. This can be obtained from the Settings section and Personal Access Token subsection.',
  'connection.accountType':
    "regex newegg regex Select 'Newegg Business' if your account is created on https://www.neweggbusiness.com.Select 'Newegg' if your account is created on https://www.newegg.com.",
  'connection.http.encrypted.apiKey':
    'regex newegg regex Please enter the unique API Key which Newegg Marketplace integration team assigned to you.Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your API Key safe. This can be obtained from the Settings section and API Key subsection.',
  'connection.http.encrypted.apiSecret':
    'regex newegg regex Please enter the unique Secret Key which Newegg Marketplace integration team assigned to you. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your key safe.',
  'connection.http.unencrypted.sellerId':
    'regex newegg regex Get Seller ID from the seller/Newegg that authorized the Newegg Marketplace API Services access to you, for each seller you are integrating for.',
  'connection.http.auth.token.token':
    'regex asana regex Enter your personal access token.Note: There are multiple layers of protection in place (including AES 256 encryption) to keep your API token safe.',
  'connection.http.auth.type':
    'regex slack regex Please select Authentication Type',
  'connection.http.auth.type':
    'regex bigcommerce regex Please select Authentication Type',
  'connection.http.auth.basic.username':
    'regex bigcommerce regex Client ID will be the Username.',
  'connection.http.auth.basic.password':
    'regex bigcommerce regex Access Token will be the Password.',
  'connection.http.auth.token.token':
    'regex bigcommerce regex This Access Token works in tandem with the Client ID.',
  'connection.http.unencrypted.clientId':
    'regex bigcommerce regex This Client ID works together with the Access Token to grant authorization.',
  'connection.storeHash':
    'regex bigcommerce regex The base api path will look something like this: https://api.bigcommerce.com/stores/123456/. In the base path, the store hash is the 123456.',
  'connection.http.encrypted.apiKey':
    'regex certify regex The API Key of your Certify account.',
  'connection.http.encrypted.apiSecret':
    'regex certify regex The API Secret of your Certify account.',
  'connection.http.chargifySubdomain':
    'regex chargify regex The subdomain of your chargify account. For example, https://mysubdomain.chargify.com.',
  'connection.http.encrypted.apiKey':
    'regex chargify regex The API key of your Chargify account.',
  'connection.environment':
    'regex docusign regex Select either Production or Demo and then click Save & Authorize that opens up the DocuSign window where you can enter your DocuSign account email ID and password to establish the connection.',
  'connection.accountType':
    'regex ebay regex Please select your environment here. Select Sandbox Accounting if the account is created on https://sandbox-quickbooks.api.intuit.com. Select Sandbox Payment if the account is created on https://sandbox.api.intuit.com. Select Production Accounting if the account is created on https://quickbooks.api.intuit.com. Select Production Payment if the account is created on https://api.intuit.com.',
  'connection.http.refreshTokenBody.user':
    'regex jet regex API User Key available from Jet under API Section-> Get API Keys',
  'connection.http.encrypted.password':
    'regex jet regex Secret Key available from Jet under API Section-> Get API Keys',
  'connection.http.baseURI':
    'regex jira regex The base URI for JIRA. For example, http://www.company.com/confluence',
  'connection.http.auth.basic.username':
    'regex jira regex The username of your JIRA account.',
  'connection.http.auth.basic.password':
    'regex jira regex To create an API token for Atlassian account:Login to your Atlassian account– >Go to User management– >Go to Your profile --> Security – > API token',
  'connection.environment':
    'regex jobvite regex Select either Production or Sandbox based on your requirement.',
  'connection.http.unencrypted.companyId':
    'regex jobvite regex The company ID of your Jobvite account.',
  'connection.http.unencrypted.api':
    'regex jobvite regex The API Key of your Jobvite account.',
  'connection.http.encrypted.secret':
    'regex jobvite regex The Secret Key of your Jobvite account.Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your user secret safe. This can be obtained from the Settings section and user secret subsection.',
  'connection.http.auth.type':
    'regex liquidplanner regex Please select Authentication Type',
  'connection.http.auth.basic.username':
    'regex liquidplanner regex Enter Username of your registered LiquidPlanner account.',
  'connection.http.auth.basic.password':
    'regex liquidplanner regex Enter Password of your registered LiquidPlanner account. The Passeord is created when the account is created.',
  'connection.http.auth.token.token':
    'regex liquidplanner regex The API Token of your LiquidPlanner account when using the Token authentication.',
  'connection.http.baseURI': 'regex magento regex The Base URI of Magento 2.',
  'connection.http.auth.token.token':
    'regex magento regex The access token of your Magento 2 account.',
  'connection.http.mailchimpDataCenter':
    'regex mailchimp regex The Data Center name provided by Mailchimp. Click Save & Authorize to open up the Mailchimp login screen where you can enter your username and password to establish the connection with your Mailchimp account.',
  'connection.instanceName':
    'regex servicenow regex The URL of your instance with ServiceNow. For example, https://mycompany.service-now.com.',
  'connection.http.auth.basic.username':
    'regex servicenow regex The username of your ServiceNow account.',
  'connection.http.auth.basic.password':
    'regex servicenow regex The password of your ServiceNow account.',
  'connection.http.auth.basic.username':
    'regex shipstation regex The API Key of your ShipStation account.',
  'connection.http.auth.basic.password':
    'regex shipstation regex The API Secret of your ShipStation account. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your API secret safe. This can be obtained from the Settings section and API secret subsection.',
  'connection.http.auth.type': `regex shopify regex Integrator.io supports the following authentication types: Basic: Select Basic if your service implements the HTTP basic authentication strategy. This authentication method adds a Base64 encoded username and password values in the "authentication" HTTP request header.Cookie: Select Cookie if your service relies on session-based authentication. Session based authentication is typically implemented by including a unique cookie into the HTTP request header. By selecting this option, the platform will automatically create and insert this cookie into every HTTP request it sends to your application.Custom: Select Custom for all other types. If you select the Custom authentication method, integrator.io will not perform any special authentication. It is up to the user to configure the HTTP request fields (method, relativeUri, headers, and body) of the import and export models to include {{placeholders}} for any authentication related values. These values can be stored in Encrypted and Unencrypted fields of this connection.Token: Select Token if your service relies on token-based authentication. The token may exist in the header, URL, or body of the HTTP request. This method also supports refreshing tokens if the service being called supports it. OAuth 2.0: Select this value if your application supports the OAuth 2.0 authentication.`,
  'connection.http.storeURL': `regex shopify regex Go to your Shopify store and you can find out the store name in the browser URL. For example - if your Shopify store URL is "https://demo-store.myshopify.com/"", then provide "demo-store" as the store name.`,
  'connection.http.auth.basic.username': `regex shopify regex Login to your Shopify store and navigate to "Apps" section. Click on the respective private app and the API key can be found next to the "Authentication" section.`,
  'connection.http.auth.basic.password': `regex shopify regex Login to your Shopify store and navigate to "Apps" section. Click on the respective private app and the password can be found next to the "Authentication" section.`,
  'connection.http.auth.token.token':
    'regex stripe regex The secret key of your Stripe account.',
  'connection.http.auth.basic.username':
    'regex twilio regex The Account Sid of your Twilio account.',
  'connection.http.auth.basic.password':
    'regex twilio regex The token of your Twilio account.',
  'connection.http.baseURI': `regex woocommerce regex Please enter baseURI of your WooCommerce account. If your endpoint is  “https://shopName.com/wp-json/wc/v1/orders”, then use "https://shopName.com" as base URL.`,
  'connection.http.auth.basic.username':
    'regex woocommerce regex The consumer key of your WooCommerce account.',
  'connection.http.auth.basic.password':
    'regex woocommerce regex The consumer secret of your WooCommerce account.',
  'connection.environment':
    'regex shipwire regex Please select your environment here. Select Sandbox if the account is created on https://beta.shipwire.com. Select Production if the account is created on https://www.shipwire.com.',
  'connection.http.auth.basic.username':
    'regex shipwire regex The username of your Shipwire account.',
  'connection.http.auth.basic.password':
    'regex shipwire regex The password of your Shipwire account.',
  'connection.http.unencrypted.apiAccessKeyId':
    'regex zuora regex Please enter Username of your Zuora account.',
  'connection.http.encrypted.apiSecretAccessKey':
    'regex zuora regex Please enter Password of your Zuora account.Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your password safe. This can be obtained from the Settings section and password subsection.',
  'connection.http.sandbox': 'regex zuora regex The Zuora account type.',
  'connection.http.encrypted.apiKey':
    'regex atera regex Please enter your API key here. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your API key safe. This can be obtained by Navigating to Admin >> API from the left hand panel.',
  'connection.http.auth.type':
    'regex hubspot regex Please select Authentication Type',
  'connection.http.auth.token.token':
    'regex hubspot regex Please enter API Key of your Hubspot Account.',
  'connection.http.auth.basic.username':
    'regex insightly regex The API key of your Insightly account.Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your API key safe. This can be obtained from the Settings section and API Keys subsection.',
  'connection.http.freshdeskSubdomain':
    "regex freshdesk regex Enter your Freshdesk subdomain. For example, in https://mycompany.freshdesk.com 'mycompany' is the subdomain.",
  'connection.http.auth.basic.username':
    'regex freshdesk regex Username can be either your Freshdesk account email used to login to your Freshdesk account, or the API key associated with your account, depending on preference.',
  'connection.http.auth.basic.password':
    'regex freshdesk regex The password of your Freshdesk account.',
  'connection.accType':
    'regex avalara regex Please select your account type here. Select Production if your account URL starts with https://admin-avatax.avalara.net/. Select Sandbox if your account URL starts with https://admin-development.avalara.net/.',
  'connection.http.acceloSubdomain':
    'regex accelo regex Please enter your subdomain here which you configured in Deployment Information page while signing up for your new Accelo account.',
  'connection.http.encrypted.accountId':
    'regex harvest regex Please enter the Harvest account ID .Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your Account ID safe. This can be obtained from the Settings section and Account ID subsection.',
  'connection.http.baseURI':
    'regex pulseway regex Please enter baseURI of your Pulseway account. If you host your own Pulseway Enterprise Server, use “https://your-server-name/api” as base URL.',
  'connection.http.encrypted.apiKey':
    'regex shiphawk regex The API Key of your ShipHawk account.Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your API Key safe. This can be obtained from the Settings section and API Key subsection.',
  'connection.http.encrypted.apiKey':
    'regex newrelic regex Please enter your API key here. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your API key safe. From the account dropdown in the New Relic UI, select Account settings > Integrations > API keys to find it.',
  'connection.http.myServer':
    'regex tableau regex Please enter your server name here which you configured while signing up for a new Tableau account.',
  'connection.http.auth.basic.username':
    'regex tableau regex Please enter the User Id/Email of your Tableau Account.',
  'connection.http.auth.basic.password':
    'regex tableau regex Please enter password of your Tableau Account. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your user secret safe.',
  'connection.http.unencrypted.contentUrl':
    'regex tableau regex The content URL is the value that in the server environment is referred to as the Site ID.',
  'connection.http.auth.token.token':
    'regex tableau regex The access token of your Tableau account.',
  'connection.http.unencrypted.siteId':
    'regex tableau regex The Site ID of your Tableau account.',
  'connection.http.encrypted.apiKey':
    'regex tesco regex The subscription key of your Tesco account. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your API key safe. This can be obtained from the Settings section and API Keys subsection.',
  'connection.http.encrypted.apiKey':
    'regex fieldaware regex Please enter your API key here. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your API key safe. This can be obtained by reaching out to FieldAware support team.',
  'connection.chargebeeSubdomain':
    "regex chargebee regex Enter your Chargebee subdomain. For example, in https://mycompany.chargebee.com/api 'mycompany' is the subdomain.",
  'connection.http.auth.basic.username':
    'regex chargebee regex The API Key of your Chargebee account.',
  'connection.http.auth.token.token':
    'regex taxjar regex Please enter your token here. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your Token safe. This can be obtained by navigating to Tokens page from the options menu on the top right corner in the application.',
  'connection.http.sandbox':
    'regex pitneybowes regex Select either Locate/Identity or Ship.',
  'connection.http.unencrypted.apiKey':
    'regex pitneybowes regex Please enter API Key of your Pitney Bowes Account.',
  'connection.http.encrypted.apiSecret':
    'regex pitneybowes regex Please enter API Secret of your Pitney Bowes Account. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your user secret safe.',
  'connection.http.auth.token.token':
    'regex pitneybowes regex The access token of your Pitney Bowes account.',
  'connection.http.encrypted.token':
    'regex shippo regex Please enter your API key here. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your API key safe. You can find your token on the Shippo API settings page.',
  'connection.instanceURI':
    'regex acumatica regex Please enter URL of your instance with Acumatica. For example, http://try.acumatica.com/isv/entity/Default/6.00.001.',
  'connection.http.unencrypted.endpointName':
    'regex acumatica regex Please enter endpoint name of your Acumatica account.',
  'connection.http.unencrypted.endpointVersion':
    'regex acumatica regex Please enter endpoint version of your Acumatica account.',
  'connection.http.unencrypted.username':
    'regex acumatica regex Please enter username of your Acumatica account.',
  'connection.http.encrypted.password':
    'regex acumatica regex Please enter password of your Acumatica account.',
  'connection.http.unencrypted.company':
    'regex acumatica regex Please enter company name of your Acumatica account.',
  'connection.http.encrypted.apiKey':
    'regex zoom regex The API Key of your zoom account.Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your user secret safe.',
  'connection.http.encrypted.apiSecret':
    'regex zoom regex The API Secret of your zoom account.Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your user secret safe.',
  'connection.http.unencrypted.clientId':
    'regex bronto regex Please enter Client ID of your Bronto Account.',
  'connection.http.encrypted.clientSecret':
    'regex bronto regex Please enter Client Secret of your Bronto Account. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your user secret safe.',
  'connection.http.auth.token.token':
    'regex bronto regex The access token of your Bronto account.',
  'connection.http.auth.basic.username':
    'regex greenhouse regex Please enter your API token here. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your API token safe. You can go to Configure >> Dev Center >> API Credential Management and from there, you can create a Harvest API key and choose which endpoints it may access.',
  'connection.http.unencrypted.userID':
    'regex greenhouse regex Please enter the Greenhouse user id used for integration here. This is required by Greenhouse for auditing purposes for all write requests and can be obtained by using List Users API.',
  'connection.http.unencrypted.adminUser':
    'regex powerbi regex Please check this if you are The Power BI Service Administrator. The Power BI Service Administrator role can be assigned to users who should have access to the Power BI Admin Portal without also granting them other Office 365 administrative access.',
  'connection.http.auth.basic.password':
    'regex mailgun regex Please enter your API key here. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your API key safe. This can be obtained from the Settings section and API Keys subsection.',
  'connection.http.procurifySubdomain': `regex procurify regex Enter your Procurify subdomain. For example, in https://celigo.procurify.com/api "celigo" is the subdomain.`,
  'connection.http.unencrypted.username':
    'regex procurify regex Please enter the Username of your Procurify Account.',
  'connection.http.encrypted.password': `regex procurify regex Please enter password of your Procurify Account.Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your user secret safe.`,
  'connection.http.generateClientIdandSecret': `regex procurify regex Please click Generate "Client Id & Secret" button to get Client ID and Client Secret of your Procurify Account.`,
  'connection.http.unencrypted.clientId':
    'regex procurify regex The client id of your Procurify account',
  'connection.http.encrypted.clientSecret': `regex procurify regex Please click Generate "Client Id & Secret" button to get Client ID and Client Secret of your Procurify Account.`,
  'connection.http.auth.token.token':
    'regex procurify regex The access token of your Procurify account.',
  'connection.http.unencrypted.applicationKey':
    'regex strata regex Please enter application key of your Strata account',
  'connection.http.encrypted.subscriptionKey':
    'regex strata regex Please enter Ocp-Apim-Subscription-Key of your Strata account.Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your private key safe.',
  'connection.http.auth.token.token':
    'regex strata regex The Access Token of your Strata account',
  'connection.http.encrypted.apiKey':
    'regex aptrinsic regex Please enter your API key here. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your API key safe. This can be obtained from Account Settings in REST API section.',
  'connection.http.coupaSubdomain':
    'regex coupa regex Please enter the subdomain of your account here which can be obtained from the login url.',
  'connection.http.auth.token.token':
    'regex coupa regex Please enter API Key of your Coupa account',
  'connection.http.microsoftDynamics365Subdomain':
    'regex microsoftdynamics365 regex Please enter your team name here which you configured while signing up for a new Zendesk account.',
  'connection.http.auth.token.token':
    'regex quip regex Please enter your API token here.Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your API Access token safe. This can be obtained from the Settings section and API Access token subsection.',
  'connection.http.auth.token.token':
    'regex oandaexchangerates regex Please enter the API Key of your OANDA account.',
  'connection.sugarcrmSubdomain': `regex sugarcrm regex Please enter your SugarCRM subdomain. For example, in https://jpeyoy4394.trial.sugarcrm.eu/http/v11_2/ "jpeyoy4394.trial.sugarcrm.eu" is the subdomain.`,
  'connection.http.unencrypted.version':
    'regex sugarcrm regex Please enter endpoint version of your SugarCRM account.',
  'connection.http.unencrypted.clientID': `regex sugarcrm regex The client_id of "sugar" will automatically create an OAuth Key in the system and can be used for "password" authentication. The client_id of "support_portal" will create an OAuth Key if the portal system is enabled and will allow for portal authentication. Other client_id 's can be created by the administrator in the OAuthKeys section in the Administration section and can be used in the future for additional grant types,if the client secret is filled in, it will be checked to validate the use of the client id.`,
  'connection.http.unencrypted.platform': `regex sugarcrm regex Defaults to "base" allows you to have custom meta-data per platform. If using a value other than "base", you should make sure it is registered using the Platform extension or configure an API platform in Administration panel.`,
  'connection.http.encrypted.clientSecret': `regex sugarcrm regex Defaults to "base" allows you to have custom meta-data per platform. If using a value other than "base", you should make sure it is registered using the Platform extension or configure an API platform in Administration panel.Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your private key safe.`,
  'connection.http.auth.token.token':
    'regex sugarcrm regex The access token of your Tableau account.',
  'connection.http.marketoSubdomain':
    "regex marketo regex Please enter your Marketo subdomain. For example, in https://591-vse-736.mktohttp.com/http/v1/activities/types.json '591-vse-736.mktohttp.com' is the subdomain.",
  'connection.http.unencrypted.clientId':
    'regex marketo regex The Client ID will be found in the Admin > LaunchPoint menu by selecting the custom service, and clicking View Details.',
  'connection.http.encrypted.clientSecret':
    'regex marketo regex The Client Secret will be found in the Admin > LaunchPoint menu by selecting the custom service, and clicking View Details. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your user secret safe.',
  'connection.http.unencrypted.userAccount':
    'regex zimbra regex The user. To load an explicit user account, specify the user in one of the following formats:<ul> <li>john.doe <pre>http://localhost:7070/home/john.doe/inbox.rss </pre> </li> <li>john.doe@mydomain.com</li> <pre>http://localhost:7070/home/john.doe@mydomain.com/inbox.rss </pre> </ul>.',
  'connection.http.unencrypted.apiKey':
    'regex grms regex GRMS assigned API key to a partner account. API key is given out by GRMS customer support team.',
  'connection.http.encrypted.apiSecret':
    'regex grms regex GRMS assigned API secret to a partner account. API secret is given out by GRMS customer support team.Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your private key safe.',
  'connection.http.auth.token.token':
    'regex grms regex The Access Token of your GRMS account',
  'connection.http.encrypted.apiKey':
    'regex retailops regex Please enter API Key of your RetailOps Account.Path to get API Key(Administration > User Manager>Select your user account>User Details pane(bottom-right),when the API key appears in its separate window, this will be your only opportunity to view/copy the entire API key. Once you close the window, you will not be able to view the entire API key again (a truncated version appears under the Credentials pane to indicate that an API key was created previously). Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your API key safe. This can be obtained from the Settings section and API Keys subsection.',
  'connection.environment':
    'regex skubana regex Please select your environment here. Select Sandbox if the account is created on https://demo.skubana.com/login. Select Production if the account is created on https://app.skubana.com/login.',
  'connection.http.unencrypted.profileId':
    'regex merchantesolutions regex ID (Profile ID) issued by Merchant e-Solutions.',
  'connection.http.encrypted.profileKey':
    'regex merchantesolutions regex API password (Profile Key) assigned by Merchant e-Solutions.',
  'connection.http.encrypted.cardNumber':
    'regex merchantesolutions regex Payment card number.',
  'connection.http.unencrypted.username':
    'regex dunandbradstreet regex Please enter username of your Dun & Bradstreet account.',
  'connection.http.encrypted.password':
    'regex dunandbradstreet regex Please enter password of your Dun & Bradstreet account.Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your private key safe.',
  'connection.http.auth.token.token':
    'regex dunandbradstreet regex The access token of your Dun & Bradstreet account.',
  'connection.accType':
    'regex adp regex Please select your account type here. Select UAT if your API Endpoint starts with https://uat-api.adp.com. Select Production if your API Endpoint starts with https://api.adp.com.',
  'connection.http.unencrypted.clientId':
    'regex adp regex Please enter the Client ID provided by ADP support service.',
  'connection.http.encrypted.clientSecret':
    'regex adp regex Please enter the Client Secret provided by ADP support service. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your user secret safe.',
  'connection.http.clientCertificates.cert':
    'regex adp regex 1. Install OpenSSL Light and configure the OpenSSL by adding an environmental variable after this we have to open the openssl.cfg file which is located in the C drive and remove the ìunstructuredName = An optional company nameî present in ì[req_attributes ]î area.2.After installation, we have to generate a CSR file (Certificate Signing Request) file using command prompt and execute the below commands. Replace yourcompanynamehere with an actual company name. Command to generate the CSR file: Openssl req -new -key yourcompanynamehere_auth.key -out yourcompanynamehere_auth.csr3.Email the .csr file that was generated to ADP Representative and we have to wait for the certificate Signing and the ADP Representative will mail us .pem file.',
  'connection.http.clientCertificates.key':
  ...
  'connection.http.clientCertificates.passphrase':
    'regex adp regex If there is any password for PFX file then it should be given here.',
  'connection.http.accountType':
    'regex faire regex Please select your environment here.',
  'connection.http.encrypted.apiKey':
    'regex faire regex Please Enter the access token got from Faire support. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your API Access token safe. This can be obtained from the Settings section and API Access token subsection.',
  'connection.http.accountType':
    'regex ware2go regex Please select your environment here. Select Staging if the account is created on https://openapi.staging.ware2goproject.com/ware2go. Select Production if the account is created on https://openapi.ware2goproject.com/ware2go.',
  'connection.http.auth.basic.username':
    'regex ware2go regex Please enter your Access Token here. This can be obtained by reaching out to Ware2Go support team.',
  'connection.http.auth.basic.password':
    'regex ware2go regex Please enter your Access Secret here. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your API Secret safe. This can be obtained by reaching out to Ware2Go support team.',
  'connection.http.unencrypted.merchantId':
    'regex ware2go regex Please enter your Merchant Id here. This can be obtained by reaching out to Ware2Go support team.',
  'connection.http.auth.basic.username':
    'regex messagemedia regex Please enter your API User. Navigate to Merchant view on left hand side and click on API keys section to find API User.',
  'connection.http.auth.basic.password':
    'regex messagemedia regex Please enter your API key here. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your API key safe. Navigate to Merchant view on left hand side and click on API keys section to find API Key.',
  'connection.http.subdomain':
    "regex vend regex Please enter your SharePoint subdomain. For example, in https://temp-portal.sharepoint.com 'temp-portal' is the subdomain.",
  'connection.http.apiEndpoint':
    'regex miva regex Please enter the API Endpoint. Under Domain Settings > Site Configuration> Base URL for Graphics we will get the Domain Name. Every store has its own unique API endpoint associated with the domain name. The format will be as follows: https://www.domain.com/mm5/',
  'connection.http.unencrypted.Store_Code':
    'regex miva regex Please enter the Store Code. Under store settings, we will get the store code.',
  'connection.xMivaAPIToken':
    'regex miva regex Please enter the API token of your account. Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your API key safe. The token is generated in the Miva admin under Users > API Tokens. Note: When we are generating token we should select the Accept Requests Without Signature, Accept Requests Without Timestamp and also we should provide IP Address as 0.0.0.0/0 and also we should provide required permissions for groups and functions.',
  'connection.accType':
    'regex target regex Please select Production for a live seller account or Sandbox if you currently want to integrate with a development staging environment.',
  'connection.http.auth.token.token':
    'regex target regex Please enter the x-seller-token provided by Target',
  'connection.http.unencrypted.x-seller-id':
    'regex target regex Please enter the x-seller-id provided by Target.',
  'connection.http.unencrypted.tpl':
    'regex 3plcentral regex Contact the 3PL Central Warehouse you are working with for this project and have them submit a request for REST API access to their 3PL Central Customer Success Manager.',
  'connection.http.unencrypted.userLoginId':
    'regex 3plcentral regex Contact the 3PL Central Warehouse you are working with for this project and have them submit a request for REST API access to their 3PL Central Customer Success Manager.',
  'connection.http.auth.token.token': `regex loopreturns regex Please enter your API key here. Log into your Loop Returns Account. Go to Settings > Developers, you can generate an API key or use an existing one.
 Please note that there are multiple layers of protection in place (including AES 256 encryption) to keep your API key safe.`,
  'connection.rest.authType': `regex shopify regex Integrator.io supports the following authentication types: Basic: Select Basic if your service implements the HTTP basic authentication strategy. This authentication method adds a Base64 encoded username and password values in the "authentication" HTTP request header.Cookie: Select Cookie if your service relies on session-based authentication. Session based authentication is typically implemented by including a unique cookie into the HTTP request header. By selecting this option, the platform will automatically create and insert this cookie into every HTTP request it sends to your application.Custom: Select Custom for all other types. If you select the Custom authentication method, integrator.io will not perform any special authentication. It is up to the user to configure the HTTP request fields (method, relativeUri, headers, and body) of the import and export models to include {{placeholders}} for any authentication related values. These values can be stored in Encrypted and Unencrypted fields of this connection.Token: Select Token if your service relies on token-based authentication. The token may exist in the header, URL, or body of the HTTP request. This method also supports refreshing tokens if the service being called supports it. OAuth 2.0: Select this value if your application supports the OAuth 2.0 authentication.`,
  'connection.rest.storeURL': `regex shopify regex Go to your Shopify store and you can find out the store name in the browser URL. For example - if your Shopify store URL is "https://demo-store.myshopify.com/"", then provide "demo-store" as the store name.`,
  'connection.rest.basicAuth.username': `regex shopify regex Login to your Shopify store and navigate to "Apps" section. Click on the respective private app and the API key can be found next to the "Authentication" section.`,
  'connection.rest.basicAuth.password': `regex shopify regex Login to your Shopify store and navigate to "Apps" section. Click on the respective private app and the password can be found next to the "Authentication" section.`,
  'connection.rest.zendeskSubdomain':
    'regex zendesk regex Please enter your team name here which you configured while signing up for a new Zendesk account.',
  'connection.rest.refreshTokenBody.user':
    'regex jet regex API User Key available from Jet under API Section-> Get API Keys',
  'connection.rest.encrypted.password':
    'regex jet regex Secret Key available from Jet under API Section-> Get API Keys',
  ...
  'connection.rest.basicAuth.password':
    'regex jira regex The password of your JIRA account.',
  ...
  'connection.rest.bearerToken':
    'regex bigcommerce regex This Access Token works in tandem with the Client ID.',
  'connection.rest.unencrypted.clientId':
    'regex bigcommerce regex This Client ID works together with the Access Token to grant authorization.',
  'connection.storeHash':
    'regex bigcommerce regex The base api path will look something like this: https://api.bigcommerce.com/stores/123456/. In the base path, the store hash is the 123456.',
  'connection.accountType':
    'regex ebay regex Please select your environment here. Select Sandbox Accounting if the account is created on https://sandbox-quickbooks.api.intuit.com. Select Sandbox Payment if the account is created on https://sandbox.api.intuit.com. Select Production Accounting if the account is created on https://quickbooks.api.intuit.com. Select Production Payment if the account is created on https://api.intuit.com.',
  'import.http.requestTypeCreate':
    'regex http regex Please specify whether the record is being created or updated using this field.',
  'import.http.requestTypeUpdate':
    'regex http regex Please specify whether the record is being created or updated using this field.',
  'import.rest.requestTypeCreate':
    'regex rest regex Please specify whether the record is being created or updated using this field.',
  'import.rest.requestTypeUpdate':
    'regex rest regex Please specify whether the record is being created or updated using this field.',
};
