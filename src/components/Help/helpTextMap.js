...
export default {
 ...
 <br>Multiple layers of protection, including AES 256 encryption, are in place to keep your API key safe. When editing this connection, you must re-enter this value each time; it is stored only when the connection is saved and never displayed as text.`,
  ...
  'looker.connection.http.instanceurl': 'Enter the instance URL for Looker. For example, if your instance is located at https://test-instance.cloud.looker.com, then your Instance URL would be <b>test-instance.cloud.looker.com</b>. If your URL includes port number, it should be specified as a part of the Instance URL. For example, <b>test-instance.cloud.looker.com:443</b>',
  'looker.connection.http.unencrypted.clientId': 'Enter your Client ID. Use the following steps to retrieve the Client ID:<br> 1. Log in to your Looker account. <br>2. Navigate to <b>Admin > Users</b>. Add a new user or edit an existing user.<br>3. Under the user, you will find <b>API3 Keys</b>. Click "Edit Keys", this will redirect to a page where you can add or view <b>API3 Keys.</b>',
  'looker.connection.http.encrypted.clientSecret': 'Enter your Client secret. Use the following steps to retrieve the Client secret:<br> 1. Log in to your Looker account. <br>2. Navigate to <b>Admin > Users</b>. Add a new user or edit an existing user.<br>3. Under the user, you will find <b>API3 Keys</b>. Click "Edit Keys", this will redirect to a page where you can add or view <b>API3 Keys.</b><br><br>Multiple layers of protection, including AES 256 encryption, are in place to keep your developer token safe. When editing this connection, you must re-enter this value each time; it is stored only when the connection is saved and never displayed as text.',
  'pandadoc.connection.http.auth.token.token': 'Enter the API key of your PandaDoc account. Use the following steps to retrieve the API key: <br>1. Sign in to the PandaDoc account. <br> 2. Navigate to <b>Profile settings -> API & Integrations -> API</b> and generate the API Token.<br><br>Multiple layers of protection, including AES 256 encryption, are in place to keep your developer token safe. When editing this connection, you must re-enter this value each time; it is stored only when the connection is saved and never displayed as text.',
