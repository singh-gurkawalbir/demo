...
'The primary reason for defining a results mapping is to specify where the ‘data’ returned by the lookup should be merged back into the source record.  You can merge ‘data’ into any existing field in the source record, or you can specify a brand new field in which case integrator.io will create the field on the fly.  By default, integrator.io will add this mapping for you, and will create a brand new field in the source record following the naming convention ‘lookupResultsN’, but it is recommended that you change this name to match the type of data being returned (i.e. relatedContacts, linkedItems, etc…) so that your source records are more intuitive to read and map later.  Though much less common, you can also merge the results field ‘errors’ back into the source record if you want to manually process errors downstream in your flow logic.',
  ...
  'runHistory.status': 'Status indicates the execution state of flow runs. Click the info icon next to the <b>Canceled</b> status to view who canceled the flow.<br />You can filter flow runs based on the status, such as completed successfully, completed with errors, failed, or canceled. If you select <b>Canceled</b> status for filtering, you can also select the Canceled by option to drill down and analyze further.',
  'runConsole.status': 'Status indicates the execution state of flow runs.<br />You can click on the "info icon" next to "Canceled" status to view whether the flow was canceled by a user or by the System.',
...
'Invite users from any integrator.io account to share your stack, which will allow them to invoke the functions in your stack from their integration flows. You may revoke their access from this screen at any time.',
  ...
  'pageGenerator.checkExistingExport': 'Select an existing resource (export, lookup, or transfer) to add to this flow.<br/> <b>Note: Any edits you make to an existing resource will also apply to all other flows that use it, so verify that no other flows will break as a consequence.</b><br/><br/>To set up a new resource, leave this box unchecked and click <b>Next.</b>',
  ...
  'pageProcessor.checkExistingExport': 'Select an existing resource (export, lookup, or transfer) to add to this flow.<br/> <b>Note: Any edits you make to an existing resource will also apply to all other flows that use it, so verify that no other flows will break as a consequence.</b><br/><br/>To set up a new resource, leave this box unchecked and click <b>Next.</b>',
