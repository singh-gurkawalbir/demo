...
'Define an ‘input filter’ here to specify which source records should get processed by the import. i.e. Records that evaluate to true are processed. Records that evaluate to false are ignored (but still passed along to downstream applications in the flow).',
  ...
  'import.v2.lookups.name': 'Enter a descriptive and unique name to identify this lookup for future use.',
  ...
  'mapping.v2.lookup.method': 'Provide the HTTP method required by your lookup application to retrieve the lookup data (options include GET, PUT, or POST). Consult your destination application\'s API documentation to determine which method is best to use for your lookup.',
