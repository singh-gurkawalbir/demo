...
'Enter your personal access token. <br>Multiple layers of protection, including AES 256 encryption, are in place to keep your Personal access token safe. When editing this connection, you must re-enter this value each time; it is stored only when the connection is saved and never displayed as text.',
  ...
  'slack.connection.http.unencrypted.scopes': 'Scopes are permissions that the Slack API defines to limit the access to your account. Select the relevant scopes from the Scopes editor. For more information on each scope, see the Slack API <a href="https://api.slack.com/scopes?filter=bot"><b>documentation</b></a>.',
