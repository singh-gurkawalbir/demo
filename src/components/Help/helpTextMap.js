...
'Define an \'input filter\' here to specify which source records should get processed. i.e. Records that evaluate to true are processed. Records that evaluate to false are ignored (but still passed along to downstream applications in the flow).',
  ...
  'mfa.enable': 'Switch the toggle ON to initiate the process to enable MFA for your profile. You must also complete the following six steps; otherwise, MFA will remain disabled. When you disable MFA,  device settings will also be disabled so that you can enable MFA at any point in future',
  'mfa.getVerificationApp': 'You can configure MFA to use any authenticator app that supports time-based one-time-password protocol (TOTP). Supported authenticator apps include, but not limited to, Google, LastPass, and Microsoft.',
  'mfa.addIntegrationIO': 'To add <a href="https://docs.celigo.com/hc/en-us/articles/360019506771">integrator.io</a> to your MFA authenticator app, you can either scan the QR code or enter the secret key manually. You must enter the password to view the secret key or QR code for your profile. Do not save these details anywhere else. You can find the secret key and QR code in your profile at any time in the future if necessary.',
  'mfa.code.verify': 'Enter the code from your authenticator app for the Celigo configuration you just completed to verify that the configuration is successful',
  'mfa.primaryAccount': 'If you lose the mobile device that has the authenticator app installed, the account owner or admin of the primary account can reset MFA credentials for your profile. Choose the account that you want to perform MFA resets.',
  'mfa.trustDevice': 'If you choose to trust the device you are using to sign in to <a href="https://docs.celigo.com/hc/en-us/articles/360019506771">integrator.io</a>, you will only be required to enter MFA credentials after 90 days. Your account owner can modify the number of days a trusted device will not require credentials.',
  'mfa.connect': 'This completes the MFA enablement process for your profile.',
  'mfa.reset': 'If you have lost access to your MFA device or you believe your secret key has been compromised, click Reset to initiate the MFA reset process.',
  'mfa.qrcode': 'View the QR code that was used to enable MFA for your profile.',
  'mfa.viewSecret': 'View the secret key that was used to enable MFA for your profile.',
  'mfa.trustedDevices': 'Click <b>Manage devices</b> to see a list of all trusted devices for your profile.',
