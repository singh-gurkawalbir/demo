    ...
    if (filteredHttpEndpoints.length) {
      ...
      filteredHttpEndpoints.forEach(httpEndpoint => {
        if (httpEndpoint.supportedBy?.type === 'export') {
          ...
          const pathParameters = httpEndpoint.pathParameters?.map(pp => ({name: pp.name, id: pp.name, description: pp.description, required: pp.required !== false, fieldType: pp.dataType || pp.fieldType || 'input', suggestions: pp.values }));
