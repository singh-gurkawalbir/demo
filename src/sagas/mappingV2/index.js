import { call, takeEvery, put, select, takeLatest, fork, delay } from 'redux-saga/effects';
...
export function* mappingInit({
  ...
  if (importRes.netsuite_da) {
    const recordType = yield select(selectors.mappingV2NSRecordType, resourceId, subRecordMappingId);
  ...
  yield put(
    actions.mappingV2.initComplete({
...
export function* saveMappings({context }) {
  ...
  const {
    ...
    recordType,
    subRecordMappingId,
  } = yield select(selectors.mappingV2);
  const generateFields = yield select(selectors.mappingV2Generates, resourceId);
  const importRes = yield select(selectors.resource, 'imports', resourceId);
  const exportRes = yield select(selectors.flowPageGenerator, flowId);
  ...
  yield put(actions.mappingV2.saveComplete());
}
  ...
  try {
    ...
    if (['NetSuiteDistributedImport', 'NetSuiteImport'].includes(importRes.adaptorType)) {
      if (
        ...
        return yield put(actions.mappingV2.previewFailed());
      }
    }

    yield put(actions.mappingV2.previewReceived(preview));
  } catch (e) {
    yield put(actions.mappingV2.previewFailed());
  }
}
...
export function* checkForIncompleteSFGenerateWhilePatch({ id, field, value = '' }) {
  ...
  if (childRelationshipField && childRelationshipField.childSObject) {
    ...
    yield put(actions.mappingV2.patchIncompleteGenerates(
      id,
      key,
      relationshipName
    )
...
export function* updateImportSampleData() {
  ...
  yield put(actions.mappingV2.updateMappings(modifiedMappings));
}

export const mappingV2Sagas = [
  takeEvery(actionTypes.MAPPINGV2.INIT, mappingInit),
  takeEvery(actionTypes.MAPPINGV2.SAVE, saveMappings),
  takeEvery(actionTypes.MAPPINGV2.PREVIEW_REQUESTED, previewMappings),
  takeLatest(actionTypes.MAPPINGV2.REFRESH_GENERATES, refreshGenerates),
  takeLatest(actionTypes.MAPPINGV2.PATCH_FIELD, checkForIncompleteSFGenerateWhilePatch),
